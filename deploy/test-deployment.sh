...
# ---------------------------------------------------------------------------
# The deployment home's OWN ancestors (DEPLOY-003).
#
# The ancestor walk used to stop at the home -- "its parents are the
# platform's" -- so a home sitting beneath a writable-by-anyone parent was
# accepted. Substituting that home substitutes every unit, contract, key and
# binary at once, and because every ownership rule in the library anchors its
# expected uid on `stat` of the home, the substituted home then serves as its
# OWN trust anchor: an attacker-authored unit with an arbitrary ExecStart
# passes the trust-input rule. Measured end to end with a foreign uid before
# this gate existed.
#
# Two gates, because either alone is a trap. The negative one is satisfied by
# a walk that refuses everything; the positive one is what proves this suite's
# own mktemp-under-TMPDIR homes still install.

above_home_root="${workdir}/above-home"
rm -rf "${above_home_root}"
mkdir -p "${above_home_root}/plain/parent/svc/.config/mcloving" \
  "${above_home_root}/sticky/parent/svc/.config/mcloving"
chmod -R go-w "${above_home_root}"
chmod 0755 "${above_home_root}" "${above_home_root}/plain" "${above_home_root}/sticky"

# NEGATIVE: a writable, NON-sticky parent above the home is refused BY NAME.
chmod 0777 "${above_home_root}/plain/parent"
if (
  # shellcheck source=deploy/bin/mcloving-deploy-lib.sh
  source "${libexec}/helpers/mcloving-deploy-lib.sh"
  require_secure_ancestors "${above_home_root}/plain/parent/svc" \
    "${above_home_root}/plain/parent/svc/.config/mcloving"
) > "${workdir}/logs/above-home-writable.log" 2>&1; then
  echo "the ancestor walk accepted a world-writable parent ABOVE the home" >&2
  exit 1
fi
grep -q "plain/parent (mode 777)" "${workdir}/logs/above-home-writable.log" || {
  echo "the writable parent above the home was not named in the refusal:" >&2
  cat "${workdir}/logs/above-home-writable.log" >&2
  exit 1
}

# POSITIVE: a STICKY world-writable parent above the home is accepted. S_ISVTX
# narrows rename to root, the directory's owner and the entry's own owner, and
# all three are judged by the ownership rule -- measured: a foreign uid's mv
# under 1777 returns EPERM, under 0777 it succeeds. Without this exemption
# every install under a /tmp-rooted TMPDIR, this suite's homes first, would
# refuse on every host. Built explicitly at 1777 rather than leaning on the
# real /tmp, so the gate states the property it tests under any TMPDIR.
chmod 1777 "${above_home_root}/sticky/parent"
(
  # shellcheck source=deploy/bin/mcloving-deploy-lib.sh
  source "${libexec}/helpers/mcloving-deploy-lib.sh"
  require_secure_ancestors "${above_home_root}/sticky/parent/svc" \
    "${above_home_root}/sticky/parent/svc/.config/mcloving"
) > "${workdir}/logs/above-home-sticky.log" 2>&1 || {
  echo "the ancestor walk refused a STICKY world-writable parent above the home;" >&2
  echo "every mktemp-under-TMPDIR deployment, this suite included, depends on it:" >&2
  cat "${workdir}/logs/above-home-sticky.log" >&2
  exit 1
}

# The chain must actually CONTAIN the directories above the home: a walk that
# reached nothing would satisfy the positive gate for entirely the wrong reason.
above_home_chain="$(
  # shellcheck source=deploy/bin/mcloving-deploy-lib.sh
  source "${libexec}/helpers/mcloving-deploy-lib.sh"
  deployment_ancestor_chain "${above_home_root}/sticky/parent/svc" \
    "${above_home_root}/sticky/parent/svc/.config/mcloving"
)"
above_home_decoded="$(printf '%s\n' "${above_home_chain}" | python3 -c '
import base64, sys
for line in sys.stdin:
    line = line.strip()
    if line:
        sys.stdout.buffer.write(base64.b64decode(line) + b"\n")
')"
grep -qxF "${above_home_root}/sticky/parent" <<<"${above_home_decoded}" || {
  echo "the home's own parent never reached the ancestor chain:" >&2
  printf '%s\n' "${above_home_decoded}" >&2
  exit 1
}
grep -qxF "/" <<<"${above_home_decoded}" || {
  echo "the home's chain did not reach / :" >&2
  printf '%s\n' "${above_home_decoded}" >&2
  exit 1
}
