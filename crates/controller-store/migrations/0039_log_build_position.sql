-- PAR-013: every log chunk carries its position in its build's commit
-- order, assigned at commit under the per-build log lock, so a follower's
-- cursor is build-scoped (the table-wide cursor_id never leaves the store)
-- and a page after a position is one indexed range read rather than a
-- re-ranking of the whole ledger. Positions count every chunk ever
-- committed for the build across all fences, so they stay stable when an
-- attempt is re-fenced.
ALTER TABLE attempt_log_chunks ADD COLUMN build_position bigint;

UPDATE attempt_log_chunks AS l
SET build_position = ranked.position
FROM (
    SELECT l2.organization_id, l2.attempt_id, l2.fence, l2.sequence,
           row_number() OVER (
...
CREATE INDEX attempt_log_chunks_build_position_idx
    ON attempt_log_chunks (organization_id, attempt_id, build_position);
