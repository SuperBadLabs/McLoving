use mcloving_dependency_resolver::{
    ...
    RepositoryConfig, RepositoryGrant, ResolutionRequest, ResolverLimits, SourceTrustClass,
    admit_request, configuration_sha256, parse_maven_lock, validate_config,
};
...
fn exact_configuration_request_plan_and_lock_are_admitted() {
    ...
    let admitted = admit_request(&config, &request, &plan, &lock, NOW).expect("admitted");
    assert_eq!(
...
fn runtime_lock_plan_and_repository_substitution_are_denied() {
    ...
    assert_eq!(
        admit_request(&config, &request, &plan, b"substituted", NOW)
