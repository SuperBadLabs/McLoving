...
| TM-025 | Mixed-epoch, tampered, incomplete, or secret-bearing Jenkins inventory is admitted as migration truth | Four typed strict-YAML families, byte-identical snapshot binding, detached SHA-256 sealing, independently sourced controller/direct-child/principal/ACL/client, per-job dependency/state-class, and per-state-class instance counts; canonical job-parent/principal/ACL-permission-generation/client-direction/job-dependency/job-state-class identity-set digests with explicit empty-group ownership bindings; mandatory state coverage for every job including retired jobs; referential and coverage reconciliation; exactly-once typed compatibility evidence for every declared library/trigger/platform/agent/toolchain requirement; typed secret consumer/taint evidence with workload-visible secrets forced unsupported; forward/rollback state-transform classification; create-new publication; and no execution authority | Digest tamper, strict-YAML alias, mixed-epoch, unknown identity, population omission and count mismatch, dependent count/set collector, same-cardinality identity, parent-edge, ACL-permission/generation, client-direction, dependency-owner, or state-owner substitution, retired/out-of-scope state omission, understated state instances, missing/duplicate/undeclared requirement evidence, incomplete coverage, unclassified secret/state behavior, workload-visible-secret downgrade, and secret-reference negative tests | COMPAT/SEC/OPS | Compromised Jenkins administrator/exporter or false owner attestation |
