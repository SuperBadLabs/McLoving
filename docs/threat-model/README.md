...
| TM-036 | A cache hit crosses tenant, project, pipeline, trust, generation, or restore boundaries; returns substituted or corrupt bytes; or lets concurrent writers replace immutable content | Standalone non-executing cache process; exact executable/configuration/key/policy/generation/restore binding; canonical domain-separated keys; no trust promotion; private FULL-synchronous strict SQLite state; per-policy byte/count/TTL quotas; atomic insert-or-idempotent-replay; byte-length and digest revalidation on every hit; deterministic LRU eviction; bounded stale cleanup; controller restore-epoch fencing; same-transaction HMAC-signed hash-chained receipts; bounded strict NDJSON and no network, repository, secret, scheduler, agent, shell, or effect authority | Sealed zero-cache Mario inventory; contained cold/hit, scope/principal/trust denial, untrusted-write/trusted-read, key/metadata/content corruption, concurrent convergence/conflict, quota/expiry/eviction/cleanup, generation/restore cold-state, audit tamper, private authority, executable substitution, duplicate/unknown/oversized frame, and standalone-process tests; protocol in `docs/architecture/CACHE_SERVICE_V1.md` | CACHE/SEC | Trusted Unix host and containing directories, controller restore-epoch owner, cache policy/operator, and shared HMAC verifier; production cache mappings, live performance, canary, cutover, rollback, and decommission remain unclaimed |
