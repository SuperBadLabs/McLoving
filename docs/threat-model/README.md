...
| TM-037 | A runner or effectful connector fabricates, suppresses, reorders, substitutes, or credentials its own destination observation; a stale observer, cursor, grant, endpoint, configuration, or response is accepted as independent effect truth | Independently deployed GET-only Linux observer per destination/effect class; executing-inode digest through `/proc/self/exe`; distinct deployment, runtime, service, issuance, configuration, request, destination-attestation, and receipt-signing authorities; exact signed request/destination/receipt bindings; private FULL-synchronous durable claim/cursor and outbound-attempt ledger; monotonic cutover and rollback generation fencing; one pending read per destination scope; strict pre/post/reconciliation predecessor chain; canonical allowlisted query; pinned CA, no redirects/proxies/implicit retry; bounded headers/body/time/freshness/rate/retry/evidence and transactional terminal-evidence retention pruning; typed closed state; case-complete full marker scan; terminal-denial release and canonical tombstone convergence with bounded transport retry; no scheduler, controller, agent, workload-secret, connector-control, write, or effect authority | Contained signed pre/post/reconciliation and exact replay; durable outage/restart, retry rate budget, evidence-quota reclamation, no-read expired replay, and success/terminal-response races against concurrent expiry; forged request/receipt, replay mismatch, phase reorder, cursor rollback, stale/malformed/oversized/substituted/secret-bearing response, timeout/outage, destination permission, grant expiry, credential/config substitution, duplicate protocol and frame bounds; sealed zero-authority Mario inventory; protocol in `docs/architecture/DESTINATION_OBSERVER_V1.md` | OBS/SEC | Trusted Linux host, deployment/configuration/credential operators, destination/CA/attestation and request authorities, destination implementation and clock; transformed secrets outside marker set; production mappings and live non-collusion remain unclaimed |
