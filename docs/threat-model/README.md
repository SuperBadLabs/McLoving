...
claimed `FOR UPDATE SKIP LOCKED` with the backoff written in the claim so
two controllers hold disjoint rows and a failed row is off the next scan
until due, settlement is keyed on the claim's attempt count so an overtaken
