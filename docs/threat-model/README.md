...
| TM-006 | Agent restart loses process identity or result | Local SQLite WAL, FULL synchronous commits, durable session epochs, verified spool metadata, Linux boot/process-birth identity, and fail-closed legacy-row migration | Forced post-terminal-commit crash/replay; matching, missing, mismatched, and legacy process-identity cancellation tests; persistent-host machine reboot follows | AGENT | Host disk corruption; non-Linux Unix recovery requires an equivalent birth identity |
