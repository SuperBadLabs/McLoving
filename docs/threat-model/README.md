...
| TM-025 | Mixed-epoch, tampered, incomplete, or secret-bearing Jenkins inventory is admitted as migration truth | Four typed strict-YAML families, byte-identical snapshot binding, detached SHA-256 sealing, independent controller/direct-child counts, referential and coverage reconciliation, typed secret consumer/taint evidence, forward/rollback state-transform classification, create-new publication, and no execution authority | Digest tamper, strict-YAML alias, mixed-epoch, unknown identity, count mismatch, duplicate reference, incomplete coverage, unclassified secret/state behavior, and secret-reference negative tests | COMPAT/SEC/OPS | Compromised Jenkins administrator/exporter or false owner attestation |
