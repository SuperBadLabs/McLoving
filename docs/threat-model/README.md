...
| TM-011 | Agent impersonates a more privileged pool | One-time enrollment, mTLS identity, session/certificate epochs, measured capabilities, and exact scheduling match between the node's durable required pool and the certificate-bound agent pool | Token replay, stale-session, rotation, revocation, and mismatched-pool claim tests | AGENT/SEC | CA compromise |
