...
| TM-037 | A runner or effectful connector fabricates, suppresses, reorders, substitutes, or credentials its own destination observation; a stale observer, cursor, grant, endpoint, configuration, or response is accepted as independent effect truth | Independently deployed GET-only Linux observer per destination/effect class; production-only HTTPS loader and feature-gated literal-loopback test boundary; executing-inode digest through `/proc/self/exe`; distinct deployment, runtime, service, issuance, configuration, request, destination-attestation, and receipt-signing authorities; revocable non-self-referential configuration identity; exact signed request/destination/receipt bindings; private FULL-synchronous durable claim/cursor and outbound-attempt ledger; fixed state-lineage lease serializes cutover/rollback with in-flight reads even across destination-scope changes; durable phase heads and physical cursor high-water across receipt pruning; one pending read per destination scope; strict pre/post/reconciliation predecessor chain; canonical allowlisted query; pinned CA, no redirects/proxies/implicit retry; bounded headers/body/time/freshness/rate/retry/evidence and transactional terminal-evidence retention pruning; typed closed state; whole-value encoded and nested-encoding marker scan; terminal-denial release and canonical tombstone convergence with bounded transport retry; no scheduler, controller, agent, workload-secret, connector-control, write, or effect authority | Contained signed pre/post/reconciliation and exact replay; durable outage/restart, retry rate budget, pre-read evidence exhaustion, retained-chain and retained-cursor denial after evidence pruning, no-read expired replay, scope-changing cutover/read serialization, and success/terminal-response races against concurrent expiry; forged request/receipt, replay mismatch, phase reorder, cursor rollback, stale/malformed/oversized/substituted/secret-bearing response, timeout/outage, destination permission, grant expiry, credential/config substitution and revocation, production loopback denial, duplicate protocol and exact frame bounds; sealed zero-authority Mario inventory; protocol in `docs/architecture/DESTINATION_OBSERVER_V1.md` | OBS/SEC | Trusted Linux host, deployment/configuration/credential operators, destination/CA/attestation and request authorities, destination implementation and clock; transformed secrets outside marker set; production mappings and live non-collusion remain unclaimed |
