# Jenkins sequential Declarative contract v2

Status: JCOMP-002C diagnostic-precedence revision. The runnable grammar and
23 preregistered fixture sources/outcomes remain those of contract v1; this
revision clarifies classification before full Groovy parsing. V1 bytes and
historical receipts remain immutable and bind their original implementations.
No new executable admission, paired execution, migration eligibility or
operational authority is granted. JCOMP-003 earns paired receipts separately.
ADR 0006 remains compile-only: Groovy source is never evaluated by the compiler.
...
during compilation. Diagnostic precedence is explicit: source byte/text bounds
and raw Unicode introducer preflight are checked first. The remaining lexer
scans in source order, reporting the first independently known malformed
protected literal or lexical exclusion. A lexical exclusion is `unsupported`
before full Groovy parsing. This lexical result states that the source is outside the
protected language, not that the whole excluded source is valid Groovy. Full
Groovy parsing follows only for lexically eligible text; recognized malformed
syntax is `rejected`. Both denial classes return no admitted executable and
schedule zero work. Unknown nested-expression syntax remains unverified when
the independent recognizer cannot establish the same status and reason.

Raw Unicode introducers are checked throughout before excluded well-formed
Unicode escapes are reported, so an earlier exclusion cannot hide a malformed
introducer. A slash beginning a parenthesized argument is a lexical exclusion;
its interior quotes are not reinterpreted as ordinary protected literals.
Flat identifier/property interpolation boundaries may be recognized solely to
locate a subsequent excluded escape in a literal segment. GStrings remain
non-executable; unknown nested-expression interiors do not borrow a denial.
The first bounded lexical failure wins; this is not a full syntax audit of
excluded Groovy. Every potentially admitted input still requires Groovy parse,
lexical/AST agreement, and independent Rust source/status/code/IR agreement.

The unchanged v1 manifest preregisters exact diagnostics for its 23 fixtures.
Those original bytes and expectations do not change with this policy revision;
new executions bind the v2 contract hash. Historical v1 EDN and receipts retain
their original contract and are not accepted as current v2 compiler responses.
