...
| Release completeness | `REL-003` | PARALLEL | `REL-001` done, `DEPLOY-001`, `SEC-005` | Package and sign every executable the deployment lane installs, including the sealed helpers that dispatch external effects, and produce the production secret-broker executable that does not exist yet; `CUTOVER-001` cannot re-read a digest that no release contains, and no credential-dependent case is eligible until the broker ships |
...
| 3 | `CASE-001` | PENDING | The first effectful production case; `CANARY-001` cannot exist without one, and the owner must designate the job before the ticket's own work can begin |
