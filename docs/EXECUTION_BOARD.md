...
|---:|---|---|---|
| 1 | `PAR-005` | ACTIVE | Dogfood: a pipeline file in this repository runs the Foundation lanes that need no user namespaces, triggered by the GitHub webhook and reporting a commit status; ten consecutive main pushes must match Foundation |
...
| Helper runtime integration | `EXEC-005` | DONE | `EXT-002` done, `DEPLOY-001`, `JCOMP-003` | Wire the sealed helpers into the product path so a submitted job can reach them; packaging a binary nothing calls ships a dormant executable, not a capability |
...
| Artifact addressing | `UI-002A` | PARALLEL | `UI-002` is done | Standalone pull request correcting the artifact addressing defect `UI-002` found and deliberately left unfixed: require a row-specific `fence` discriminator on metadata, download and upload-commit surfaces so every listed duplicate-name row stays downloadable; the deferred server-rendered UI chain (`UI-003` onward) still depends on it, but the defect is an API/CLI contract and does not wait on that rewrite. Nothing is proved yet. |
...
| Recovery log publication | `AGENT-008` | PARALLEL | `PAR-010` merged | Standalone pull request reordering agent startup recovery so journaled logs upload before a recovered cancellation completes; no protocol or fencing change |
| Container runtime binding | `AGENT-009` | PARALLEL | `PAR-011` merged as `0eb949ba` | Standalone pull request binding the effective podman storage identity into every container invocation and the journaled container context, launching under an agent-owned pinned containers configuration with an empty mounts file, and refusing `#`-prefixed environment names for container stages; no protocol change |
| Source acquirer throughput and retained state | `AGENT-013` | PARALLEL | `PAR-005` merged | Standalone pull request reading blobs in one batched git process instead of one process per file, reclaiming a killed acquisition's retained state on the next attempt, and fetching the requested commit by object id when the branch tip has moved; no protocol change |
| Dogfood ingress modes and drift comparison | `DOGFOOD-001` | PARALLEL | `PAR-005` merged | Standalone pull request for the dogfood deployment scripts: persist the GitHub webhook `heman-up.sh` registers in public-hook mode and disable it when the deployment restarts in bridge mode, so a push is never delivered twice under two delivery ids; and make `verify-lanes.py` compare the test command a `run-verified-rust-test.sh` invocation wraps instead of classifying the whole invocation as scaffolding. Filed from PR #152 review round 12 past the correction cap; no product code |
| Artifact collection work budget | `AGENT-012` | PARALLEL | `PAR-014` merged as `9208e4e2` | Standalone pull request precompiling a stage's artifact patterns once and bounding the walk's aggregate matching work, probing a zero-length collected file for growth before its header-only stream closes, opening each collected file only when its upload starts rather than pinning the whole set, moving the controller's staging finalization (fsync) off the runtime's worker threads, publishing a declared set atomically so a file that changes mid-set leaves nothing of the set visible, checking cancellation and authority between reads of the digest pass, giving the client deadline headroom over the server's receive budget for the commit, bounding the join of the streaming reader, moving the controller's per-frame writes off the runtime's workers, reconciling the in-flight ledger with rows an upload has already registered, recording the controller's per-object and store quota refusals as named artifact refusals, and resuming a pending object's existing staged or committed copy on an exact retry instead of staging a second; wire change confined to the artifact stream |
| Reserved log chunk custody | `AGENT-011` | PARALLEL | `PAR-013` merged as `0e2cf213` | Standalone pull request keeping a copy of every reserved live-log chunk under the agent's journal directory until its receipt, so a workload that unlinks, renames, truncates or overwrites its own spool can neither strand a reserved sequence nor pin the attempt in recovery; no protocol change |
| Commit-status reconciliation and NAT64 prefixes | `CTRL-007` | PARALLEL | `PAR-004` merged as `bf47e751` | Standalone pull request reconciling a commit status against the target after the quiet interval, so a write the target applied after the local deadline is corrected, and letting the operator name the deployment's network-specific NAT64 prefixes so their embedded IPv4 addresses are decided by the address policy; no protocol change |
| Sequential planner artifact refusal | `CTRL-006` | PARALLEL | `PAR-014` merged as `9208e4e2` | Standalone pull request making the sequential planner refuse a stage that declares artifacts, as it refuses a container stage, instead of planning nodes with the declarations dropped; no protocol change |
| Log quota accounting | `CTRL-005` | PARALLEL | `PAR-013` merged as `0e2cf213` | Standalone pull request keeping each attempt's committed-log byte count as one row updated under the per-attempt log lock the append already holds, so the byte quota check is a read rather than a sum over every prior chunk; no protocol change |
| Source publication hardening | `AGENT-010` | PARALLEL | `PAR-012` merged as `e22a94ed` | Standalone pull request making the pre-publication verification of a retained checkout descriptor-relative to the held acquisition handle, aligning the manifest bound with the admitted repository limits, and bounding the final durability syncs by the step deadline; no protocol change |

<!-- board-graph: allow PAR-014 ~ UI-002A -- PAR-014 is the CLI artifact-download consumer of the two-parameter form; UI-002A changes that addressing grammar and records the consumer-generation disposition, but the tickets share no code ordering: PAR-014 already shipped the consumer and UI-002A is the later contract correction -->
<!-- board-graph: allow AGENT-008 ~ AGENT-009 -- both are parallel hardening follow-ups filed from parity review; AGENT-008 reorders recovery's log upload and AGENT-009 pins the podman store identity, touching disjoint recovery code with no ordering between them -->
<!-- board-graph: allow AGENT-008 ~ AGENT-010 -- parallel hardening follow-ups from parity review touching disjoint code: recovery log upload versus checkout publication; no ordering between them -->
<!-- board-graph: allow AGENT-009 ~ AGENT-010 -- parallel hardening follow-ups from parity review touching disjoint code: podman invocation binding versus checkout publication; no ordering between them -->
<!-- board-graph: allow AGENT-008 ~ AGENT-011 -- parallel follow-ups from parity review touching disjoint code: recovery log upload of finished spools versus custody of reserved live chunks; no ordering between them -->
<!-- board-graph: allow AGENT-009 ~ AGENT-011 -- parallel follow-ups from parity review touching disjoint code: podman invocation binding versus custody of reserved live chunks; no ordering between them -->
<!-- board-graph: allow AGENT-010 ~ AGENT-011 -- parallel follow-ups from parity review touching disjoint code: checkout publication versus custody of reserved live chunks; both open files without following links but share no code; no ordering between them -->
<!-- board-graph: allow AGENT-008 ~ AGENT-012 -- parallel follow-ups from parity review touching disjoint code: recovery log upload versus the artifact collector's matching budget; no ordering between them -->
<!-- board-graph: allow AGENT-009 ~ AGENT-012 -- parallel follow-ups from parity review touching disjoint code: podman invocation binding versus the artifact collector's matching budget; no ordering between them -->
<!-- board-graph: allow AGENT-010 ~ AGENT-012 -- parallel follow-ups from parity review touching disjoint code: checkout publication versus the artifact collector's matching budget; both open files without following links but share no code; no ordering between them -->
<!-- board-graph: allow CTRL-005 ~ CTRL-006 -- parallel follow-ups from parity review touching disjoint code: the store's log quota accounting versus the sequential planner's admission of artifact stages; no ordering between them -->
<!-- board-graph: allow CTRL-005 ~ CTRL-007 -- parallel follow-ups from parity review touching disjoint code: the store's log quota accounting versus the notification worker's commit-status reconciliation; no ordering between them -->
<!-- board-graph: allow CTRL-006 ~ CTRL-007 -- parallel follow-ups from parity review touching disjoint code: the sequential planner's admission of artifact stages versus the notification worker's commit-status reconciliation; no ordering between them -->
<!-- board-graph: allow AGENT-011 ~ AGENT-012 -- parallel follow-ups from parity review touching disjoint code: custody of reserved live-log chunks versus the artifact collector's matching budget; no ordering between them -->
<!-- board-graph: allow AGENT-011 ~ AGENT-013 -- parallel follow-ups from parity review touching disjoint code: custody of reserved live-log chunks versus the source acquirer's blob reads and retained state; no ordering between them -->
<!-- board-graph: allow AGENT-012 ~ AGENT-013 -- parallel follow-ups from parity review touching disjoint code: the artifact collector's matching budget versus the source acquirer's blob reads and retained state; no ordering between them -->
<!-- board-graph: allow AGENT-010 ~ AGENT-013 -- parallel follow-ups from parity review touching disjoint code: publication of a retained checkout after a crash versus the acquirer's blob reads and the state a killed acquisition leaves; no ordering between them -->
<!-- board-graph: allow AGENT-008 ~ AGENT-013 -- parallel follow-ups from parity review touching disjoint code: the multi-step envelope's step bookkeeping versus the source acquirer's blob reads and retained state; no ordering between them -->
<!-- board-graph: allow AGENT-009 ~ AGENT-013 -- parallel follow-ups from parity review touching disjoint code: container stage containment versus the source acquirer's blob reads and retained state; no ordering between them -->
<!-- board-graph: allow AGENT-010 ~ AGENT-013 -- parallel hardening follow-ups from parity review touching disjoint code: checkout publication versus acquirer throughput/retained state; no ordering between them -->
<!-- board-graph: allow AGENT-011 ~ AGENT-013 -- parallel hardening follow-ups from parity review touching disjoint code: reserved log chunk custody versus acquirer throughput/retained state; no ordering between them -->
<!-- board-graph: allow AGENT-012 ~ AGENT-013 -- parallel hardening follow-ups from parity review touching disjoint code: artifact collection work budget versus acquirer throughput/retained state; no ordering between them -->

### Parity lane

| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba`, `PAR-012` as `e22a94ed`, `PAR-001` as `327a032a`, `PAR-013` as `0e2cf213`, `PAR-014` as `9208e4e2` and `PAR-004` as `bf47e751`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
...
| AGENT-008 | PENDING | AGENT-007, PAR-010 | Publish a crashed attempt's already-journaled logs before recovery completes its cancellation. Today agent startup sends reconciliation (which completes the cancellation and terminalizes the journal row) before it replays finalizations, so the spool descriptors a multi-step attempt journaled for its completed steps are reclaimed without ever being uploaded, and the controller receives only the generic cancellation result; single-step attempts have always lost their logs on this path. Acceptance: on restart after a crash mid-attempt, every journaled log descriptor is published under the attempt's fenced authority before the cancellation completion is sent and before the row leaves `cancelling`; a shipped-binary test crashes the agent during the second of two steps and reads step zero's output from the build afterwards; replay after a lost upload response stays idempotent. Proof: the crash test's log assertion, plus `mcloving logs` on such a build showing step zero's chunks. |
| AGENT-009 | PENDING | PAR-011 | Bind the effective podman storage identity into container launch and reap. PAR-011 journals the runtime path and the `HOME`/`XDG_RUNTIME_DIR` values a container was launched under and refuses recovery-time proof under a different context, but rootless podman also selects its container store through `$HOME/.config/containers/storage.conf` (`graphroot`, `rootless_storage_path`, `driver`); an edit to that file between launch and reap points `rm --ignore` and `container exists` at a different, empty store, whose "absent" answer would falsely terminalize an attempt whose container is still writing into the workspace. Filed from PAR-011 review at the ten-round cap; the file is service-account-owned configuration on the agent host, the same trust as the agent binary, so this is a hardening of the proof, not an open exposure. Acceptance: every podman invocation the agent makes (probe, launch, teardown reap, recovery reap) pins the store explicitly (`--root`, `--runroot`, `--storage-driver`, or an equivalent resolved from `podman info` at session open) and the journaled container context carries that identity; recovery refuses to prove absence when the effective store differs from the journaled one; a test edits the storage configuration between launch and recovery and shows the attempt parked rather than terminalized. Two further findings from the same review's twelfth round are folded in here rather than filed separately, since all three bind the podman invocation to agent-owned inputs: (b) a host or user `mounts.conf` adds implicit bind mounts to every plain `podman run`, so the launch must run under an agent-owned, mode-400 `CONTAINERS_CONF` the deployment installs with an empty mounts file, proven by a test that installs a `mounts.conf` entry and shows the path absent inside the step; (c) podman's `--env-file` treats a line beginning with `#` as a comment, so container-stage admission and the executor both refuse environment names beginning with `#`, alongside the existing `=` and multiline refusals, proven by an admission test. Proof: the storage-configuration test, the mounts.conf test, the `#`-name refusal test, and a journal row showing the store identity in `container_context`. |
| AGENT-013 | ACTIVE | PAR-005 | Source acquirer throughput and retained state. The dogfood checkout of this repository (2 121 files, 33 MB, a four-second depth-one clone) took the sealed acquirer past a fifteen-minute step timeout: after the fetch it reads every blob through its own `git cat-file` process under containment, about eight files a second. The acquisition the timeout killed left its stage, runtime and git-exec directories, its claim and the transport tree behind, and the next acquisition of the same binding answered `state_unavailable` at startup until an operator removed them by hand. Filed from PAR-005's first dogfood runs. Acceptance: blobs are read in one batched git process per acquisition (`cat-file --batch` or an archive) so a tree of this size materializes in seconds, with the per-blob bounds kept; a killed acquisition's retained state is recovered or refused by name by the next acquisition of the binding, never left for an operator; and a checkout whose branch has moved past the requested commit fetches that exact object (GitHub serves reachable commits by id) rather than answering `revision_mismatch`, so a push superseded before its build ran still builds what was pushed, as Foundation does. Proof: the dogfood checkout step under one minute on the owner's host, and an integration test that kills an acquisition mid-materialization and checks the next one succeeds. |
| DOGFOOD-001 | PENDING | PAR-005 | Dogfood ingress modes and drift comparison. `scripts/dogfood/heman-up.sh` registers the repository webhook at GitHub in public-hook mode but keeps no record of it, so a restart in bridge mode leaves the hook live and a push reachable both ways is delivered twice under two delivery ids (the hook's UUID and the bridge's push-event id), two builds for one push: persist the hook id in the state directory and disable it whenever the bridge starts. `scripts/dogfood/verify-lanes.py` classifies a whole `bash scripts/run-verified-rust-test.sh ... cargo ...` invocation as scaffolding, so Foundation changing the wrapped package, test target or flags while the lane keeps the old one still reports `dogfood-lanes-ok`: normalize the wrapper and compare the wrapped command; and the comparison runs one way only (every Foundation command must be in the lane), so a step removed from Foundation leaves an obsolete lane command that can fail the dogfood build alone: compare the normalized command sets in both directions. The hook record must also cover a changed public base URL (patch the persisted hook rather than register a second one). Filed from PR #152 review rounds 12 and 13, past the ten-round correction cap; deployment scripts only, no product code. |
| AGENT-012 | PENDING | PAR-014 | Bound the artifact collector's aggregate matching work. The collector (PAR-014) matches every walked entry against every declared pattern with a table sized by the pattern's and the path's segment counts, which bounds one pair but not the walk: at the legal maxima (sixteen declarations of thirty-two patterns, 65 536 entries, patterns of some two hundred alternating `**` and literal segments against depth-32 paths) the aggregate is on the order of 10^11 cell evaluations with a table allocated per pair, run synchronously without a cancellation check while the lease keeps renewing, so a pathological declaration can occupy an agent for a long time after its steps. Also: a zero-length collected file skips the streaming pass's growth probe, so a detached writer appending between the digest pass and the header-only stream's close leaves the empty object accepted. Filed from PAR-014 review past the ten-round cap. Acceptance: a stage's patterns are compiled once into segment lists shared by matching and descent; the walk charges every cell evaluation against a per-attempt matching budget and refuses the set by name (`artifact_refused:matching_budget`) when it is spent, checking the execution-cancellation token between entries; the streaming pass probes for growth before closing a header-only stream for a zero-length file and records `changed_length` like any other. Also: the collector keeps every collected file open in one vector, so a set near the 1 024-object bound cannot complete under the default 1 024 soft descriptor limit (`EMFILE` surfaces as an unreadable-entry refusal), and the controller runs the staging writer's `sync_all` on a runtime worker thread, so several large uploads finishing at once on a slow filesystem can stall lease renewals and unrelated traffic. Acceptance, continued: the collector records each file's identity and reopens it, re-identified, only when its upload starts, holding at most one open at a time, and the agent's deployment unit declares the descriptor limit it needs; the controller finalizes a staging writer on the blocking pool; a declared set is published atomically: every object of the set is staged and registered pending under the attempt before any is marked available, and a refusal mid-set (a file changed under its read, an unreadable entry found late) aborts every pending object of the set, so a refused set leaves nothing visible, as the collector's contract and the remote-work gate assert; the digest pass checks the execution-cancellation and authority-lost tokens between reads (or runs under a cancellable deadline), so a cancellation that lands while a large file is hashed on a slow filesystem ends the pass promptly instead of after the whole read; the client's upload deadline exceeds the server's receive budget by the commit's headroom (or one end-to-end server deadline covers receive, finish, registration and availability), so a slow upload the server accepts is never a client timeout; the streaming reader's join is bounded, since a read stalled on a slow filesystem cannot observe the closed channel until it returns; the controller writes each frame on the blocking pool like the finalization; and a stream's in-flight ledger charge is released, or the database snapshot taken before it, once its row is registered, so a concurrent upload is not charged the same bytes twice (two 128 MiB uploads must fit a 256 MiB quota); and the controller's `ResourceExhausted` answers for an object past `MCLOVING_MAX_OBJECT_BYTES` or the store's total quota are recorded as named refusals (`artifact_refused:object_quota:<path>`) rather than ending the session, or the per-object limit is negotiated to the agent so it refuses before uploading; and an exact retry of a `pending` object resumes its existing publication claim or recognizes its already committed digest before staging, so near the store's total quota the retry does not need a second full copy to recover the pending metadata. Proof: a unit test with sixteen declarations of thirty-two maximal alternating patterns against a workspace of 65 536 entries finishing within the budget's bound, one with a zero-length file appended to after collection refused by name, and one collecting 1 024 files under a soft descriptor limit of 1 024 successfully. |
| AGENT-011 | PENDING | PAR-013 | Keep reserved live-log chunks in agent custody until receipted. The live tail (PAR-013) reserves a chunk in the journal and re-reads its bytes from the step's spool at send and at replay; the spool sits under the workload-writable workspace, so a step that renames or unlinks its own `stdout.log` after a reservation and before the controller's receipt leaves recovery with a reserved sequence whose bytes exist nowhere, and recovery then treats the absent stream as empty and completes the cancellation with that sequence missing from the ledger; a step that truncates or overwrites the spool instead leaves recovery failing the publisher's coverage or digest check on every session, so the attempt never terminalizes (recovery already refuses a permission or I/O failure and restores workload-revoked access; a confirmed absence and a spool rewritten under a reservation remain). Filed from PAR-013 review past the ten-round cap. Acceptance: reserving a chunk writes its bytes to an agent-private file under the journal directory (`O_NOFOLLOW`, mode 0600, fsynced before the journal row) that is removed on receipt and with the reservations at retire; send and replay read the copy, not the spool, and the digest re-check compares the copy; recovery of an interrupted step whose spool is absent publishes every unreceipted reservation from its copy and reports the unstreamed tail as lost in the attempt's diagnostics rather than terminalizing silently. Proof: a gate test whose workload unlinks its spool after the first acknowledged chunk and the agent crashes with a second chunk reserved, showing the reserved sequence published on restart with the ledger dense. |
| CTRL-007 | PENDING | PAR-004 | Reconcile commit statuses against the target. The notification worker orders writes to one commit status durably: an attempt is marked in flight before its request, a later outcome for the same status is delayed past that attempt's local deadline, and a stale settlement re-posts the newer outcome. The local deadline bounds how long the worker waits, not how long GitHub may take to apply a request whose body it already received, so a request the target applies after the quiet interval can still become the latest status. Filed from PAR-004 review past the ten-round cap. Acceptance: after the quiet interval a delivered `github_status` row is reconciled by reading the target's current status for its repository, commit and context and, when the recorded outcome is not what stands there, posting it once more, bounded by the attempt count; the reconciliation is claimed like a delivery so two controllers never reconcile one row at once. Also filed here: on a deployment whose NAT64 translator uses an RFC 6052 network-specific prefix inside global unicast, a synthesized address is allowed without its embedded IPv4 address being examined; the follow-up lets the operator name such prefixes (`MCLOVING_NOTIFICATION_NAT64_PREFIXES`) so the embedded address decides, as the well-known prefix already does. Proof: an integration test whose sink applies a timed-out older write after the newer build's post, reconciled to the newer outcome, and a unit test decoding a configured prefix to a refused private address. |
| CTRL-006 | PENDING | PAR-014 | Refuse artifact-bearing stages in the sequential planner. `plan_sequential_build` builds one execution node per literal-shell step from a Jenkins-compiled layout and passes an empty declaration list to the execution envelope, so a v1.8 pipeline whose stage declares artifacts that reaches it is planned with the declarations silently dropped: the build can succeed without publishing what the pipeline's semantic digest says it publishes. No such pipeline reaches the planner today (the Jenkins compiler emits no declarations; Lane J has not started), so this is a latent gap, not exposure. Filed from PAR-014 review past the ten-round cap. Acceptance: the planner refuses a stage with declarations the way it refuses a container stage, with a diagnostic naming the stage, until stage-level collection is carried into its per-step nodes. Proof: a unit test planning a layout whose stage declares an artifact and reading the refusal. |
| CTRL-005 | PENDING | PAR-013 | Account committed log bytes incrementally. Every `append_log_in_session` sums `octet_length(content)` over all of the attempt's prior chunks to enforce the 64 MiB per-attempt byte quota; with live streaming (PAR-013) an attempt may commit up to 262 144 small chunks, so the sum makes streaming quadratic in the number of chunks on the shared PostgreSQL controller. Filed from PAR-013 review past the ten-round cap; the quota itself is enforced correctly today, so this is cost, not exposure. Acceptance: the committed byte count of an attempt is one durable value updated in the same transaction as the append under the per-attempt log lock (migration with a backfill from the existing chunks), the quota check reads it, and a redelivered identical chunk does not count twice; a store test appends past the quota and shows the refusal with the count unchanged. Proof: a PostgreSQL store test that commits a few thousand small chunks and shows each append's plan reads no chunk row beyond its own conflict check. |
| AGENT-010 | PENDING | PAR-012 | Harden the publication of a retained checkout. PAR-012 verifies the acquired tree with the acquirer's own routine immediately before moving it, but that routine opens `output_root/<acquisition_id>` by pathname while the move uses a separately held directory descriptor, so a concurrent same-account process could swap what is verified for what is moved; the verifier also refuses any manifest above the acquirer's 64 MiB tool-output bound even when acquisition admitted the repository, and the final `sync_all` calls after the move are not bounded by the step deadline. Filed from PAR-012 review past the ten-round cap; the exposure needs a hostile process running as the agent's own account, which is the agent's existing trust boundary, so this hardens the proof rather than closing an open exposure. Acceptance: verification reads every manifest entry and inventory relative to the held acquisition and tree descriptors (`openat`, `fstatat`, `O_NOFOLLOW`) with no pathname reconstruction; acquisition bounds the manifest it writes by the same admitted file and path limits verification accepts, or verification accepts every manifest acquisition can write; the final syncs are bounded by the step's remaining time and a cancellation, with an unbounded sync reported as a refused publication. Proof: a test that swaps the pathname-addressed acquisition for a different tree between verification and the move and shows the move refused; a test that admits a manifest above 64 MiB and shows it verified; a test that stalls the final sync past the deadline and shows the step refused rather than hung. Two further findings from the same review's twelfth round are folded in here: (d) the recovery walk bound is journaled beside `attempts.acquisition_directory`, or the recovery fallback covers the maximum admitted tree, so a binding removed or shrunk after a crash cannot make the reclaim impossible on every later session; (e) the synthetic zero-budget checkout record (`attempt_output_budget_exhausted`) carries its `output_limit_exceeded` termination through finalization instead of the preceding process outcome. Proof for those: a recovery test with the binding removed after the crash showing the tree reclaimed, and a terminal summary showing `output_limit_exceeded` for a checkout that never spawned. |
...
| UI-002A | PENDING | UI-002 | Address an artifact record by the key the store actually uses. `crates/controller-api/ui/app.js`'s `downloadArtifact` builds `.../artifacts/content?attempt_id=...&name=...` and omits `fence`, so activating Download on one of two rows that differ only by fence can deliver the other row's bytes, under the requested name, with nothing on screen indicating the request was ambiguous. **This is an API surface defect, not a client fix.** `ArtifactQuery` in `crates/controller-api/src/lib.rs` carries exactly `attempt_id` and `name`; `find_artifact` resolves them with `.find()` and takes the first match; `build_artifacts` orders by `n.node_key, a.ordinal, o.name` with no fence tiebreak, so which of two matching records wins is not determined by the query at all. Against that, `attempt_objects` is keyed `PRIMARY KEY (organization_id, attempt_id, fence, kind, name)` — **fence is part of the storage key**, so the public addressing grammar is strictly weaker than the key of the table it reads, and the operations built on it — `getArtifactMetadata`, `downloadArtifact`, and `POST .../artifact-uploads/{upload_token}/commit` — inherit the ambiguity. The listing operation already returns `fence` in `ArtifactResponse`: the API hands every authorized caller an identity it then refuses to accept. The CLI is in the same position — `mcloving artifact-download` takes `build`, `attempt` and `name` and nothing else — so this is not a browser-client defect and is filed as its own work rather than folded into a client ticket, on the rule `UI-007` states for itself: a route is an authority surface and an interface is not. `UI-007` is also the wrong host on scope, because its artifact entries are uploads and the separate `GET .../artifacts/metadata` operation, not the addressing grammar both share. Two existing records must be reconciled by this ticket rather than left standing. `docs/architecture/EXTERNAL_READ_CONSUMERS_V1.md` publishes `?attempt_id=...&name=...` as the external-consumer artifact contract while addressing **log** records in the same table by fence, so the inconsistency sits inside a published contract and the parameter cannot be made required without a consumer-generation decision. And `OPS-003`'s closure record asserts that public controller journeys "stage, commit, list, inspect, and download artifacts bound to the exact tenant/build/node/attempt/fence/name/digest/size/media type" — a caller cannot request that exact tuple through the shipped download operation, because the operation has no way to express the fence. **Provenance:** found during `UI-002` (PR #138, merged as `1c2d4ea1`) and deliberately left unfixed there — that ticket is bounded to test infrastructure, versioned evidence and the minimal served-client repairs its browser and accessibility claims require, and changing download addressing is none of those. Before this ticket the deferral had no durable in-repo record: it is stated in the `1c2d4ea1` commit message and in the PR #138 review reply on `scripts/ui-browser/gate.py`, and `crates/controller-api/examples/ui_browser_fixture.rs` tells the reader the limitation is "filed separately" — a forward reference to a filing that did not exist until this row. `UI-002`'s residual-risk section does not carry it, which is the one place a later reader would look. The reproduction already ships: that fixture serves two `report.txt` records under one attempt at fences 1 and 2 with different digests and byte counts, on purpose, and the `UI-002` gate's `artifact_download_delivers_content` assertion deliberately drives delivery through the uniquely named `build.log` row instead, because a download of the ambiguous name would assert whatever the fixture chose to answer rather than what the shipped controller would. **Acceptance, fail-closed.** (1) The ambiguity is reproduced against the real store before anything is changed — two artifact records under one attempt sharing a name and differing only by fence, and a `/artifacts/content` request that returns a record the caller did not ask for. A test that cannot construct that pair has not reproduced the defect and does not satisfy this clause; the fixture pair is a rendering, not evidence about the store. (2) The addressing grammar is decided explicitly and the decision recorded: the route accepts a **row-specific discriminator** — `fence`, which is already part of the storage key and already returned in `ArtifactResponse` — and **refuses** a request that matches more than one record without it. A “define a total order” alternative is refused: under a shared `attempt_id`+`name`, total-order alone maps both listed downloads to identical query parameters and therefore always returns the same record, so activating the other row continues to deliver the wrong bytes and cannot satisfy clause (5). Silently resolving an ambiguous request to one of its matches is refused as an outcome, and **returning the first row of an unordered result is refused specifically**. (3) Whatever is decided binds `getArtifactMetadata`, `downloadArtifact`, `POST .../artifact-uploads/{upload_token}/commit` (which today also calls `find_artifact` without the request's fence after publishing the new record, so its 201 may describe an older same-name row), the OpenAPI document, the CLI's `artifact-download` / upload-commit client, and the served client in the same pull request. A client that can express fence against a route that cannot, or a route that can against a CLI that cannot, leaves the defect reachable through the other path and does not close this ticket. (4) `docs/architecture/EXTERNAL_READ_CONSUMERS_V1.md` is amended in the same pull request and states the compatibility disposition for existing consumers of the two-parameter form; adding a required parameter to a published read contract is a consumer-generation decision and is named as one rather than made in passing. `OPS-003`'s download-binding sentence is either made true by this ticket or amended to what the operation can actually do; it is not left as written while the operation cannot express the tuple it names. (5) The `UI-002` browser gate gains an assertion that drives delivery through the **ambiguous pair itself** and requires the record whose row was activated, which is the assertion that ticket could not write; `artifact_download_delivers_content` keeps its unambiguous row so the existing coverage is not traded away. The same acceptance surface includes a **duplicate-name commit response** assertion: committing a second artifact that shares a name with an older fence must return the newly committed record (fence and digest), not the older one. All are mutation-proved, captured in a versioned baseline bound to this ticket's exact source, with the `UI-002` initial and accepted baselines retained unchanged. (6) The `ui_browser_fixture.rs` comment's "filed separately" forward reference is repointed at this ticket or removed, so no source comment asserts a filing that a reader cannot find. **Bounded deliberately:** artifact addressing only. No new artifact operation, no retention or lifecycle change, and no authorization change — the fence is already carried in `ArtifactResponse` and already visible to every caller authorized to list. |
| UI-003 | DEFERRED | UI-002, UI-002A | Decide the browser trust and session model before any server-rendered UI is written. **Server-rendered HTML and a bearer-token API are in tension, and the tension is a threat boundary rather than a coding detail.** `UI-001`'s property is that the client uses only the documented public API, that the controller is the sole authorization authority, and that no privileged backend path, embedded secret, or client-side authorization claim exists. Rendering tenant data into HTML in the controller changes that shape in three ways that must be decided together. First, the controller gains a second representation of every resource and with it an **output-encoding boundary it does not have today**: every rendered value is an injection site, and JSON-to-DOM and server-rendered HTML do not fail the same way. Second, the browser must carry credentials on authenticated fragment requests, and the credential choices have distinct properties. The current client reads the token input into its JavaScript context and leaves the input value in the DOM; it is not a memory-only implementation. A **cookie session** and **bearer transport through an Authorization header** are both choices to assess. Bearer transport does not require embedding credentials in server-rendered HTML or DOM attributes, but it needs client code to attach the header: ordinary initial navigation cannot do so, and authenticated progressive behavior without scripting must be limited or use a separately decided mechanism. Assess XSS exposure, CSRF, token-entry handling, JavaScript/DOM copies and persistent browser storage separately for the chosen design; a transport choice alone does not settle those risks. The existing OIDC `SessionResponse` returns both an `access_token` and a longer-lived `refresh_token`, so the decision must explicitly cover storage, lifetime, rotation/revocation and XSS/CSRF exposure for both credentials. Moving only the access token into a cookie or memory leaves the refresh credential unresolved. Existing OIDC start/callback/refresh/logout routes do not decide the browser session model. Record the navigation and progressive-behavior costs without forcing a cookie session or token-in-document design. Third, whichever is chosen, **authorization must not be re-implemented in a view layer** — the HTML path has to run the same authorization code as the JSON path, and that has to be proved rather than asserted. The measured constraint the decision must respect: the shipped policy is `default-src 'none'; script-src 'self'; style-src 'self'; connect-src 'self'; img-src 'self' data:; form-action 'self'; base-uri 'none'; frame-ancestors 'none'`, existing executing checks enforce parts of that CSP together with structural HTML, landmark, form and client-contract assertions; they do not establish rendered browser behavior. The shipped policy forbids inline script, inline style and evaluated strings. Any client library must therefore be vendored and served from the origin rather than a CDN, inline event-handler attributes are unavailable, and every library feature that evaluates a string is unavailable unless the policy is weakened. Acceptance, decision-shaped and fail-closed. (1) An architecture record that decides the session model, states the alternative it rejected and the cost it accepted, and does not defer the CSRF question to implementation. (2) A threat-model entry for the output-encoding boundary, with the register row and the attribution it requires. (3) The `UI-001` property restated in terms the chosen model makes true, so a later reader is not left comparing a new design against a sentence written for the old one. (4) The decision must preserve the shipped CSP for this lane, matching the unchanged-policy requirements of `UI-004` and `UI-006`. Any proposed relaxation requires a separate explicit owner decision and a coordinated board revision before implementation; it cannot be authorized implicitly by this ticket. (5) If a client library is chosen, it is pinned to an exact version and digest with its supported-upgrade story recorded, including whichever fallback applies if that version is withdrawn. **Bounded deliberately:** this is a decision and records it; no source, schema, route, asset or CSP change lands here, and it grants no authority. An implementation that changes the served UI is a separate ticket and must not be folded into this one, for the same reason `GROOVY-001` separates deciding the compatibility plane from building it. |
