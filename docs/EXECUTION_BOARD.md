...
**Jenkins compatibility M1: general Linux sequential Declarative pipelines
with literal `sh` steps, proved through paired Jenkins and McLoving execution.**

The owner lifted the full September freeze on 2026-09-06; that decision and
its completed thaw receipts remain in force. On 2026-09-08 the owner selected
broader pipeline support, chose sequential shell as the first slice, and
confirmed implementation should resume with this milestone as the priority.
See `docs/handoffs/CURRENT.md`. Normal protected-merge requirements remain.

Run the board and closure verifiers for live totals. The September 8 local
reorganization observation is 112 tickets, 86 `DONE`, and 26 remaining.
Existing
legacy compiler admission recognizes one exact hello-world source identity; the
historical differential gate certifies 1/228 sources, while the public migration
package admits none and the owner-private package admits one disabled case.
Those bounded receipts do not establish general Declarative support. McLoving
is not release-ready and has no production authority.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `JCOMP-002B` | ACTIVE | `JCOMP-002A` is merged and verified complete; deliver contained workspace continuity before `JCOMP-003` earns the paired execution claim |

Success means at least ten supported-input fixtures plus explicit rejection
...
before the paired campaign. `JCOMP-001`, `JCOMP-002` and `JCOMP-002A` are `DONE`; `JCOMP-002B` is `ACTIVE`, and `JCOMP-003` remains `PENDING`.
...
Runtime inspection found two prerequisites beyond compilation: current product
admission rejects multiple steps per stage, and workspaces are attempt/fence
scoped and removed after finalization. The separate step-execution and workspace
tickets below must land before the paired campaign; compiler acceptance alone
cannot establish runnable support. Workspace lifecycle/non-collision belongs to
...
|---|---|---|---|---|
| Sequential Declarative support | `JCOMP-002B` -> `JCOMP-003` | SERIAL | `JCOMP-002A` verified complete at `c2aaa0d`; `JCOMP-002B` selected after exact-main Foundation/Windows verification | One contract, then compiler/admission, sequential step execution, contained workspace continuity, and paired product execution; one PR per ticket and no production authority |
...
| Production qualification | `CASE-001` -> `CANARY-001` -> `MIG-008` | SERIAL | `CANARY-000` protected-main verification complete; `EXT-002` merged as `03a1f5d`; `SEC-005`, `EXEC-005`, and `SECRET-002` complete, because the case certifies a runtime all three change. The `CASE-001` -> `CANARY-001` transition additionally gates on `CANARY-002`, `REL-003`, `DEPLOY-002`, and `CASE-002`: the first real external effect must not be granted before the ceremony driver exists, before the release that signs it is complete, and before that release is proved installable | Close the runtime effect-integration gap, then the owner designates and fully certifies one real effectful case before the separately authorized one-action ceremony |
| Ceremony driver | `CANARY-002` | SERIAL | `EXT-002` merged, `CASE-001` | Producer side of the ceremony: signed gate-receipt assembly against contained fixtures only; the merged `CANARY-000` verifier is the acceptance oracle |
| Authority transfer | `CUTOVER-001` -> `ROLLBACK-001` -> `RECUTOVER-001` -> `DECOM-001` -> `MIG-009` | SERIAL | `MIG-008`, `SEC-005`, `REL-003`, `DEPLOY-002` | Strict one-at-a-time transition chain with a fresh pre-action threat receipt at every authority change |
...
| Case recertification | `CASE-002` | SERIAL | `CASE-001`, `DEPLOY-002` | Re-run and re-seal the case's differential, MIG-006/MIG-007 joins, and shadow replay against the deployment that actually ships; the CASE-001 evidence necessarily predates it. Declaring the case ineligible does not close the ticket: it stays open until an owner-designated replacement case is certified end to end and recertified against the deployment |
...
| Release completeness | `REL-003` | SERIAL | `REL-001` done, `DEPLOY-001`, `SEC-005`, `SECRET-002`, `EXEC-005`, `CANARY-002`, `CASE-001` | Package and sign every executable the deployment lane installs, including the sealed helpers that dispatch external effects; `CUTOVER-001` cannot re-read a digest that no release contains. Completeness is per platform as well as per executable: `REL-001` covers `private-linux-x86_64` only, so a Windows canary needs a Windows release artifact or explicit ineligibility. No credential-dependent case is eligible until the broker ships |
...
| Lease survival | `AGENT-007` | PARALLEL | `EXEC-004` is done | Standalone runtime correction; retain ACTIVE until independent review, protected merge and exact-main Foundation/native Windows verification. JCOMP-003 waits for this exact runtime. Review record `docs/evidence/AGENT-007_SECURITY_REVIEW.md` |
| Web interface | `UI-002` -> `UI-003` -> `UI-004` -> `UI-005` -> `UI-006` -> `UI-007` -> `UI-008` -> `UI-009` | SERIAL | `UI-001` and `API-002` are done | One pull request per ticket, in this order, none folded together. The chain is serial rather than parallel because every ticket after `UI-003` rewrites the same rendered views and the same served assets, which the working rules require serializing before implementation rather than reconciling after review. `UI-002` earns the browser-journey claim `UI-001` already made and cannot support, and captures the baseline the rest are measured against; `UI-003` decides the session and output-encoding boundary a server-rendered interface moves and ships no implementation, on the `GROOVY-001` pattern; `UI-004` acquires the owner-selected library under the unchanged policy; `UI-005` proves denial parity between representations; `UI-006` migrates the six views that exist against the `UI-002` baseline; `UI-007` gives an interface to the half of the public surface that has none; `UI-008` replaces polling; `UI-009` re-earns the accessibility and viewport contracts against what actually ships. Nothing is proved by any of them yet |

### Dispatch discipline

The selected milestone is `JCOMP-001` -> `JCOMP-002` -> `JCOMP-002A` -> `JCOMP-002B` -> `JCOMP-003`.
Advance only after the predecessor is merged and verified on protected `main`.
The subsequent production-readiness track begins with `EXEC-005`, then
`SECRET-002`, `SEC-005`, and `CASE-001`; its existing certification and
one-action authorization gates remain mandatory. `MIG-005A` and `MIG-007`
