...
| Cache | `CACHE-001` | SERIAL | `DEP-001` merged and protected main verified | Standalone tenant/trust-class cache boundary PR; next post-DEP implementation slot |
...
| Dependencies | `DEP-001` | SERIAL | `SCM-001` is done | Architecture-repair candidate `9e3235ce0c100413a9c1ad613fb783d405a9e9d2` replaces transient and durable dynamic directory trees with exclusive regular archives, closes the two repeated creation-to-open namespace races, marker-scans the authenticated permanent commit before persistence and after replay loading, carries one marker scanner across the complete transport plan, carries one stateful guard across the exact generated archive serialization, routes final archive-sync failure through exact cleanup or poisoned ambiguity, serializes complete fetches, and fences every external poison transition through the same slot. All 120 focused tests pass, including production-wired cross-artifact, header-to-payload, sync-failure, archive/root metadata, real non-file/device/zero-inode validation, second-fetch denial, overlapping-fetch, and external-poison proofs; predecessor documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passed workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, thread closure, merge, and protected-main verification remain required. The complete audit contains 136 actionable findings across 141 important seams. Mario's sealed denominator still grants zero workload dependency or repository authority, so cache, production dependency, and later authority-transfer gates remain separate. |
...
PR #35 keeps `DEP-001` active through exact-head checks, review, merge, and
protected-main verification. `CACHE-001` remains pending until a subsequent
board-advance change records that verified protected-main closure.

1. `DEP-001` — finish the atomic-archive closure, exact protected checks,
   independent review, thread resolution, merge, and protected-main verification;
   then advance to the separate `CACHE-001` boundary.
...
3. `EXT-001` — independent effect-authority connector and prerequisite for
   `SECRET-001`, `DIFF-003`, and later effect-bearing qualification.
...
| DEP-001 | ACTIVE | SCM-001, SEC-003 | Implement policy-bound workload dependency resolution for Maven/npm/PyPI and other admitted ecosystems. Bind repository identity and trust policy, package coordinate, exact version, lockfile, transitive graph, content and signature/attestation digests, resolver/toolchain, credential grant, and audit provenance; mutable or unresolved coordinates are ineligible. Prove missing, repository/package/graph substitution, compromised mirror, untrusted-source, credential leak, offline/replay, and later-resolution denial before any dependency-resolving canary or cutover. Architecture-repair candidate `9e3235ce0c100413a9c1ad613fb783d405a9e9d2` replaces both dynamic directory hierarchies with one exclusive contiguous transport archive and one sealed atomic publication archive, marker-scans the authenticated permanent commit before persistence and after replay loading, carries one marker scanner across the complete transport plan, carries one stateful guard across the exact generated archive serialization, routes final archive-sync failure through exact cleanup or poisoned ambiguity, serializes complete fetches, and fences every external poison transition through the same slot. It passes 120 focused tests, including production-wired cross-artifact, header-to-payload, sync-failure, archive/root metadata, real non-file/device/zero-inode validation, second-fetch denial, overlapping-fetch, and external-poison proofs; predecessor documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passed workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, resolution of fixed threads, merge, and protected-main verification remain required before `DONE`; the current audit is 136 actionable findings across 141 important seams. Closure evidence: `docs/evidence/DEP-001_SECURITY_REVIEW.md`. Mario's sealed denominator contains zero admitted workload dependencies, so production dependency resolution, canary, cutover, rollback, and decommission authority remain separately gated. |
| CACHE-001 | PENDING | DEP-001, SEC-002, OPS-003 | Implement tenant/project/pipeline/trust-class-isolated dependency and build caches with canonical keys, immutable generation/content digests, explicit read/write policy, bounded size/expiry, atomic publication, and auditable provenance. Prove cold and valid-hit behavior, corruption and key/generation substitution rejection, untrusted-write/trusted-read denial, concurrent publication, rotation, eviction, cleanup, and restored-state behavior before any cache-dependent canary or cutover. |
...
carries the dependency-resolution architecture-repair candidate after its exact
implementation head passed the local focused gate; protected checks, independent
review, thread resolution, merge, and protected-main verification remain open.
Mario's sealed
...
persistent-Windows campaign is closed. `DEP-001` remains active until PR #35 is
merged and exact protected main is verified; only a subsequent board advance may
activate `CACHE-001`.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `DEP-001` | ACTIVE | `CACHE-001`, `DIFF-003`, and cache-dependent qualification |
