...
1. `MIG-007` — `MIG-005A` and the immutable `MIG-006` three-evidence-set
   aggregation are complete, so reproducible migration-package generation is
   the active serial ticket.
...
MIG-007 is now the active serial ticket.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `MIG-007` | ACTIVE | `SHADOW-001` and production qualification |
