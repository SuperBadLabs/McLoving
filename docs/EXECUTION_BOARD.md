...
PR #35 is merged at protected-main commit `82a51082`; exact-head checks,
independent review, thread closure, and post-merge Foundation and Windows
verification are complete. This board-advance change closes `DEP-001` and
activates the separate `CACHE-001` boundary.

1. `CACHE-001` — implement the standalone tenant/project/pipeline/trust-class
   cache boundary without importing resolver, scheduler, runner, or production
   authority.
...
Protected `main` at `82a51082` includes the completed compiler, shared-library,
...
resolver, and persistent-Windows work. DEP-001's exact PR head passed all nine
protected checks and fresh independent review, all fixed threads were resolved,
and the squash commit passed post-merge Foundation and Windows verification.
Mario's sealed
...
persistent-Windows campaign and DEP-001 are closed. `CACHE-001` is the active
successor in slot 1.
