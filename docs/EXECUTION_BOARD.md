...
| Source acquisition | `SCM-001` | PARALLEL | `SEC-003`, `AGENT-004` are done | Exact-head review reopened the isolated boundary for credential-marker completeness, sealed executable/CA snapshots, a descriptor-bound dynamic-runtime closure, credential-free single-rule multi-address endpoint pinning, one absolute monotonic credential deadline shared by the parent and child modes, pinned HTTPS-helper and internal Git-child lookup, refusal of ignored object filters, filtered transfer with live process-group quota enforcement, Unicode-normalized collision denial, durable claim-first immutable-root/directory replay, cleanup, and late-publication withdrawal; dependent work remains gated until the replacement head passes review, protected checks, and protected-main merge |
...
1. `SCM-001` — close the reopened exact-head source-acquisition findings and
   merge the independently reverified contained-source trust boundary; `DEP-001`
   remains next but is not dependency-ready before that merge.
...
| SCM-001 | ACTIVE | SEC-003, AGENT-004 | Implement isolated live source acquisition for checkout, Git, submodule, and credentialed repository steps. Bind provider, repository identity, authenticated ref and exact revision, fork and trust policy, submodule graph, sparse/depth options, checkout implementation, scoped short-lived credential grant, and resulting content/provenance digests. Prove later-commit delivery, ref substitution and untrusted-fork denial, credential non-disclosure, replay resistance, bounded network/filesystem authority, cleanup, and differential checkout truth before any source-dependent canary or cutover. Exact-head review reopened closure with forty-seven actionable findings plus this board gate, and hosted verification exposed a multi-address endpoint-rule defect; all are implemented and under replacement exact-head verification, so `DEP-001`, `SECRET-001`, and `DISC-001` remain blocked until protected-main merge. |
...
provisioner boundary, and persistent-Windows work. PR #34 carries the reopened
isolated source-acquisition boundary; closure and protected-main inclusion remain
gated on replacement exact-head checks, review, and merge. Mario's sealed inventories contain no
...
|---:|---|---|---|
| 1 | `SCM-001` | ACTIVE | `DEP-001`, `DIFF-003`, and source-dependent qualification |
