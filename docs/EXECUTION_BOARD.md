...
| PAR-004 | PENDING | PAR-014 | Notifications. The build outcome derivation emits one terminal outbox event per build; a worker with a delivery ledger and exponential backoff delivers it at least once to per-pipeline targets declared in the pipeline: a GitHub commit status using a token file, and a generic signed HTTPS webhook. The existing outbox publish primitive is untouched. Acceptance: a commit status appears on GitHub for a real build; a sink that fails twice then succeeds shows three attempts and one delivery; two workers deliver once. Proof: `gh api` on the commit status of a real commit showing the McLoving context with the build's outcome and a target URL that opens the build. |
