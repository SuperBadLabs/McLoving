...
|---:|---|---|---|
| 1 | `PAR-001` | ACTIVE | GitHub webhook receiver feeding the existing trigger path; `PAR-013` live logs and `PAR-014` artifacts follow |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-001` -> `PAR-013` -> `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba` and `PAR-012` as `e22a94ed`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
