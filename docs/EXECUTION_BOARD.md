...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Identity | `AUTHZ-001` | SERIAL | `IDP-001` is done | Separate security-reviewed PR; authorization starts from the merged identity model |
...
1. `AUTHZ-001` — now unblocked by the completed identity substrate; discovery
   and external client migration wait behind the exact policy mapping.
...
|---:|---|---|---|
| 1 | `AUTHZ-001` | PENDING | Identity-dependent discovery and client migration |
