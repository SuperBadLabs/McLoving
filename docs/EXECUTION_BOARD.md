...
|---:|---|---|---|
| 1 | `EXEC-005` | ACTIVE | Wire sealed helpers through the product path before SECRET-002, SEC-005 and CASE-001 |
...
| Web interface | `UI-002A` -> `UI-003` -> `UI-004` -> `UI-005` -> `UI-006` -> `UI-007` -> `UI-008` -> `UI-009` | SERIAL | `UI-002` is done | One pull request per ticket, in this order, none folded together. The chain is serial rather than parallel because `UI-002` may need minimal client repairs and later implementation works on the same rendered views and served assets; the working rules require serializing that shared surface before implementation rather than reconciling after review. `UI-002` is **done**: the browser-journey claim `UI-001` could not support is now earned by an executing gate, the minimal served-client repairs it permitted are made, and both baselines it had to preserve exist -- the initial failing one at `docs/evidence/ui-002-browser-v1/README.md` and the accepted repaired one successors compare against at `docs/evidence/ui-002-browser-v2/README.md`; `UI-002A` corrects the artifact addressing defect `UI-002` found and deliberately left unfixed, and leads the chain rather than waiting for `UI-007` because it changes a route's query grammar and a published external-consumer contract, which `UI-007` explicitly files as separate work; deferring it would have `UI-005` prove representation parity and `UI-006` migrate the artifact view with the ambiguity intact; `UI-003` decides the session and output-encoding boundary a server-rendered interface moves and ships no implementation, on the `GROOVY-001` pattern; `UI-004` acquires the owner-selected library under the unchanged policy; `UI-005` proves denial parity between representations; `UI-006` migrates the six views that exist against the accepted `UI-002` baseline, retained unchanged; `UI-007` gives an interface to public operations missing from the current client; `UI-008` replaces polling; `UI-009` re-earns the accessibility and viewport contracts against what actually ships. Nothing is proved by any of them yet. `REL-003` waits for `UI-009`, so the complete UI chain and its final browser proof precede packaging and subsequent deployment/canary evidence; earlier case/security receipts must be revalidated where the UI changes invalidate their bound runtime. |

### Dispatch discipline

The selected milestone is `JCOMP-001` -> `JCOMP-002` -> `JCOMP-002A` -> `JCOMP-002B` -> `JCOMP-002C` -> `JCOMP-003`.
