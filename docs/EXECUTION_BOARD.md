...
1. `SCM-001` — independent contained-source boundary and prerequisite for
...
read/write migration gates, isolated external-input adapter, and
persistent-Windows work. PR #33 carries the completed scoped dynamic-agent
provisioner boundary and its independently reviewed executable receipt;
protected-main inclusion remains gated on that PR's closure-only exact-head
checks, review, and merge. Mario's sealed provider inventory contains no
admitted dynamic provisioner, and production provisioning, canary, cutover,
rollback, and decommission authority remain separately gated. The
persistent-Windows campaign is closed.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `SCM-001` | ACTIVE | `SECRET-001`, `DISC-001`, and `DEP-001` |
