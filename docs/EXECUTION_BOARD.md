...
| Live inputs | `INPUT-001` | PARALLEL | `SEC-003`, `AUDIT-001` are done | The isolated typed read-only adapter is under final credential-file admission review; any real production input, canary, cutover, rollback, or decommission claim remains separately gated |
...
1. `INPUT-001` — close the credential-file ownership/mode finding, complete
   exact-head review, and seal the executable security receipt.
...
read/write migration gates, and persistent-Windows work. INPUT-001 is under
final credential-file admission review in PR #32; protected-main inclusion
remains gated on exact-head checks, review, and merge. Mario's sealed runtime-dependency
...
|---:|---|---|---|
| 1 | `INPUT-001` | ACTIVE | `PROV-001`, then `DIFF-003` and dynamic-agent qualification |
