...
1. `PROV-001` — implement the scoped out-of-process dynamic-agent provisioner
   boundary and retained lifecycle/cleanup evidence.
...
read/write migration gates, and persistent-Windows work. PR #32 carries the
completed isolated external-input adapter boundary and its independently
reviewed executable receipt; protected-main inclusion remains gated on that
PR's closure-only exact-head checks, review, and merge. Mario's sealed runtime-dependency
inventory contains no admitted live external input, and production caller/input
cutover remains separately gated. The persistent-Windows campaign is closed.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `PROV-001` | ACTIVE | `DIFF-003` and dynamic-agent qualification |
