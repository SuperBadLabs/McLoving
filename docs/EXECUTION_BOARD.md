...
| MIG-006 | PENDING | MIG-003, MIG-004, MIG-005 | Run the same committed corpus through the pinned Jenkins oracle and McLoving in separate, independently tested deny-authority sandboxes with bounded CPU/memory/time/output, no network or host mounts, and no secrets, database, agent, scheduler, or controller authority. Execute every bounded input equivalence class and success/failure scenario declared by MIG-002, including condition and matrix branches plus timeout, retry, cancellation, and post paths; compare stage order, step/effect arguments, post behavior, terminal outcome, workspace artifact digests, bounded normalized stdout/stderr with stream identity and explicit gaps, and TEST-001 normalized suite/case/retry/flaky outcomes with exact retained-source provenance. Publish a stable mismatch taxonomy and regression budget while keeping parse reach, native runnable coverage, actionable migration, and certified equivalence as distinct metrics. |
