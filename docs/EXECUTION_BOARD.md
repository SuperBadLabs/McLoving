...
does not reopen the product behavior those historical runs verified.
...
- The app-bound `Foundation` and `Windows` aggregate checks, strict exact-head
  synchronization, review-thread resolution, exact commit, clean worktree,
  admin enforcement, conversation resolution, and a fresh protection-rule
  readback must be verified before protected-main merge. Both required checks
  must be reported by GitHub Actions application id `15368`.
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Merge authority | `CI-003` | PARALLEL | `CI-002` done | Make complete Foundation and classified Windows outcomes the only app-bound merge-authority contexts; this grants repository merge gating only, no product or production authority |
