...
| Release provenance | `REL-001` | PARALLEL | Owner ceremony inputs authorized; v2 two-phase protocol repair in review | The isolated builder, SBOM, signing, and verifier contracts are implemented. Before creating the production key, v2 must remove the v1 self-reference by signing only predeclared log/key/anchor requirements and binding the final envelope to post-sign Rekor evidence plus an independently timestamped evidence digest. Then rebuild protected main and execute the authorized HeMan-only ceremony |
...
PR #55 is merged at protected-main commit
`5e02566ac3f76d8261b6578f71ccb438bd51bda3`; all nine exact-head checks,
clean independent exact-head review, resolution of all nineteen review threads,
and post-merge Foundation and Windows verification are complete. This
board-advance change closes `DIFF-002` without advancing an unready successor.

1. `REL-001` — owner authorization fixes `McLoving v0.1.0`, profile
   `private-linux-x86_64`, a new release UUID, one-time genesis policy,
   HeMan-only `release-key:production:v1` custody, public Rekor secondary
   attestation and an independent public timestamp anchor. The active gate is
   review/merge of the v2 two-phase protocol repair, followed by a fresh
   protected-main artifact and the exact ceremony.

`DIFF-003` remains queued until the `REL-001` signing and transparency ceremony
is complete. Every other named parity-substrate prerequisite, including
`DIFF-002`, is frozen.
...
| REL-001 | ACTIVE | OPS-002, AUDIT-001 | Produce trusted McLoving release provenance from reviewed protected-branch source through an isolated pinned builder. Bind source/tree, toolchain and builder image, dependency lock and SBOM, tests and policy gates, archive/component digests, version/profile, signer identity, and transparency/audit evidence; sign the immutable release and verify it before deployment. Prove source, dependency, builder, artifact, signature, transparency, timestamp-anchor, and rollback-target substitution denial before any production canary or cutover. Current state: `docs/architecture/RELEASE_PROVENANCE_V2.md` records the contained builder and the two-phase protocol that signs predeclared transparency requirements, then binds the final envelope to Rekor evidence and binds that canonical evidence digest to an independent public timestamp. Owner authorization fixes `McLoving v0.1.0`, profile `private-linux-x86_64`, a new UUID, one-time genesis, HeMan-only `release-key:production:v1` custody, public Rekor secondary attestation, and the independent anchor. Closure requires the v2 repair to merge and pass protected-main checks, a fresh exact-head artifact, and retained successful ceremony evidence; no repository-only substitute may fabricate those authorities. |
...
SECRET-001, and DIFF-002 campaigns are closed. `REL-001` remains active; its
owner-selected signing and transparency inputs are authorized. The remaining
gates are the reviewed v2 two-phase protocol repair, a fresh protected-main
artifact, and successful execution/retention of the HeMan signing, public Rekor
and independent timestamp ceremony.
