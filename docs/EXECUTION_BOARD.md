...
| W4-B | MIG-001, MIG-002, MIG-003 | ACTIVE | Isolated compiler boundary, exact inventory-derived Jenkins corpus, and first deterministic Declarative translation |
...
`W4-B` is the current batch. `MIG-001` establishes the isolated, deny-authority
compiler worker against this exact core/plugin profile. `MIG-002` commits and
stratifies the exact secret-scanned corpus and oracle expectations.
`MIG-003` admits only the first deterministic Declarative subset, binds exact
source/profile/compiler digests, and requires independent Rust reparse and
validation. No compiler result grants scheduler, credential, agent, trigger,
connector, effect, canary, or cutover authority. `WIN-001`, `WIN-002`, and
`WIN-003` remain independently active for their production and
persistent-Windows-host gates.
