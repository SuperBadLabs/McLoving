...
|---:|---|---|---|
| 1 | `PAR-012` | ACTIVE | Checkout step through the sealed source-acquirer; closes `EXEC-005`; `PAR-001` webhook follows |
...
| Source publication hardening | `AGENT-010` | PARALLEL | `PAR-012` merged | Standalone pull request making the pre-publication verification of a retained checkout descriptor-relative to the held acquisition handle, aligning the manifest bound with the admitted repository limits, and bounding the final durability syncs by the step deadline; no protocol change |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-012` -> `PAR-001` -> `PAR-013` -> `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5` and `PAR-011` as `0eb949ba`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
