...
| Deployment revalidation | `DEPLOY-002` | SERIAL | `DEPLOY-001` done, `REL-003` | Prove the release that actually ships can be installed, started, upgraded, and rolled back on a clean host, on every platform still eligible; the DEPLOY-001 proof covers an artifact that predates the helpers, broker, driver, and verifier |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Deployment validation surface | `DEPLOY-003` | SERIAL | `DEPLOY-001` done | Bound the lane's validation surface to the decided trust boundary: ask the manager instead of modelling it, validate the load-path union instead of resolving the selection, and teach the suite to exercise the query path it currently skips |
...
evidence receipt existed, so the board correctly held it `ACTIVE` for a day
after its code was on protected `main`. Both receipts now exist and it is
`DONE`. Reviewing `DEPLOY-001` measured a containment gap on its own
...
|---:|---|---|---|
| 1 | `DEPLOY-003` | ACTIVE | `DEPLOY-001` is closed at `586230b` with `TM-050` and its evidence receipt; this ticket bounds the validation surface that closure cost 162 findings to reach, and `SEC-005` inherits its trust-boundary decision |

One slot is filled, and that is the honest state rather than a gap to be
padded. `DEPLOY-001` is closed, which makes `SEC-005`, `SECRET-002`, and
`EXEC-005` dependency-ready; the slot nonetheless holds `DEPLOY-003`, because
`SEC-005` is about to specify containment for the same lane and inherits its
trust-boundary decision, and adopting that decision after `SEC-005` is written
would be more expensive than adopting it before. Every remaining ticket depends
