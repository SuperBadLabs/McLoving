...
- No implementation ticket that changes authentication, authorization,
  protocol, compiler/execution, connector, agent-pool, supply-chain,
  deployment, migration, or decommissioning boundaries may become `DONE`
  until `docs/threat-model/README.md` is reviewed and updated with the
  affected threats, mitigations, verification evidence, and residual risks;
  unchanged sections require an explicit reviewed no-change receipt.
...
| W4-B | MIG-004, MIG-005, AUTHZ-001, MIG-006 | PENDING | Versioned mappings, shared-library/scripted boundaries, migrated-job authorization parity, and differential certification |
...
| DISC-001 | PENDING | TRIG-001, SCM-001, AUTHZ-001 | Implement Multibranch Pipeline and Organization Folder indexing/discovery. Bind parent configuration, provider/organization/repository identities, branch/PR discovery and trust/filter strategies, Jenkinsfile path, exact discovered revision and provenance, child identity/configuration, orphan policy, and audit lineage. Prove new/updated/deleted branch and PR discovery, trusted and untrusted forks, filtering, parent reconfiguration, duplicate/reordered webhook plus periodic reindex, restart/outage catch-up, child authorization, orphan retirement, and rollback restoration before any parent or child canary or cutover. |
