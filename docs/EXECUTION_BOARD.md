...
| Commit-status reconciliation | `CTRL-007` | PARALLEL | `PAR-004` merged | Standalone pull request reconciling a commit status against the target after the quiet interval, so a write the target applied after the local deadline is corrected; no protocol change |
