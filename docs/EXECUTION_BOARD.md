...
| Lease survival | `AGENT-007` | DONE | `EXEC-004` is done | The agent no longer treats a renewal it could not get an answer to as one the controller refused. Proved by denying the network rather than the renewal, both edges, with the retry bounded by the held lease and the cancellation ordered before the controller's own recorded expiry; the pre-ticket tree is red on both new gates. Closure `docs/evidence/AGENT-007_SECURITY_REVIEW.md` |
...
| AGENT-007 | DONE | AGENT-004, AGENT-005, EXEC-003, EXEC-004 | Ride out a controller outage the lease already covers. `EXEC-004` proved authority holds WHILE work happens; this is the other half of the same property, and it fails. In `bins/agent/src/worker.rs`, `renew_lease` treats every renewal it cannot get an ANSWER to as authority lost: an RPC timeout records `renewal_timeout`, a transport `Err` records `renewal_transport_failure`, and both then cancel `execution_cancellation`, cancel `authority_lost`, and return — killing a running step that still holds an unexpired lease. At shipped defaults (`MCLOVING_AGENT_LEASE_SECONDS` 30, `MCLOVING_AGENT_RENEW_MILLISECONDS` 5000) that discards about 25 seconds of held authority on the first failure. **Measured, 2026-09-09, protected main `84705da`, HeMan, production remote-agent lane over mTLS.** A single-step build was interrupted by `SIGKILL` to the controller 4 s into a ~25 s step, with the agent process and the step's own process untouched, and the controller restarted 3 s later; the agent's lease was 300 s, so 296 s of it remained. McLoving **aborted, 2/2**, the agent naming the cause on stderr as `lease_lost_during_execution: renewal_transport_failure; cancelling the running step`. The same scenario against Jenkins 2.568.3 **succeeded, 2/2**: it reattached to its detached step process and the build finished. Neither engine ever duplicated a step in any scenario of that campaign, so this is availability rather than correctness — but a controller restart is the ordinary shape of an upgrade, and today every one of them destroys in-flight work. **The store already permits waiting; only the agent refuses to.** Agent sessions are a table in the controller's own database, so a session epoch survives a controller process restart rather than being held in its memory; `requeue_one_expired` reclaims an attempt only where `lease_expires_at <= clock_timestamp()`; and the renewal statement itself requires `lease_expires_at > clock_timestamp()`. While the lease stands, nothing else can take the attempt. Acceptance, fail-closed. (1) Separate a REFUSAL from an UNREACHABILITY, and change only the second. A renewal the controller answered — `renewal_rejected`, `renewal_session_stale`, a mismatched session epoch, or `accepted` false — must keep cancelling at once and keep the cause it already reports; that is fencing and it does not weaken by one case. Only a renewal that got no answer at all becomes retryable. (2) Retry until the lease the agent HOLDS actually lapses, and then cancel with a NEW named cause distinct from today's, so an operator can tell "the controller was unreachable and the lease ran out" from "the controller said no". The step must not outlive that deadline, because past it the attempt is requeuable and a second runtime may already hold it. (3) Prove both edges with a test that denies the NETWORK rather than the renewal: an outage shorter than the remaining lease must finish the step and terminalize success with no requeue and no `reconciliation_required` parking, and an outage longer than it must cancel and name the new cause. Gate two of `bins/agent/tests/long_step_lease.rs`, which denies renewal at the controller, must survive unchanged in meaning and stay red if removed. (4) Prove the retry cannot race reclamation: with the outage ending AFTER the lease lapsed and the attempt already claimed elsewhere, the returning agent must be refused, and the test must name which check refuses it rather than merely observing that nothing bad happened. (5) State the retry cadence and the bound on it. An unbounded retry against a controller that never returns is a step that runs forever; the lease is the bound, and the ticket must show that it is the bound actually enforced rather than the one intended. (6) Every check is mutation-proved to `HYG-002`'s standard: removed, it turns a named test red. (7) A receipt that re-runs the measured scenario above against the fix and records both arms. **Bounded deliberately:** this does not add resume after AGENT death — the campaign's both-processes-killed scenario aborts on McLoving in 8 s and fails on Jenkins after 619 s, and reattaching to an orphaned step is a separate question this ticket does not open. It changes no fencing rule, no controller code, no schema, and no protocol, and grants no production or Jenkins authority. Exploratory provenance only: the campaign's Jenkins arm has no sealed provenance and its evidence is input to this ticket, never a receipt for it. **Closed.** The distinction the code collapsed is now one predicate: a renewal the controller ANSWERED still withdraws authority immediately and keeps its cause, and a renewal that produced no usable answer is retried -- at the smaller of one second and the agent's own cadence -- until the held lease runs out, then cancels under `renewal_unanswered_until_expiry`, a cause distinct from every answered refusal. `renewal_went_unanswered` deliberately reuses `retryable_authority_transition`, the predicate every other authority-bearing RPC in the same file already uses; the renewal task was the only caller that disagreed with it, and because it owns `authority_lost` its disagreement also tore down the retry loops of the publication and start-work paths. **Two new gates deny the NETWORK rather than the renewal**: an outage inside the held lease must change nothing about the build AND must be visible in the agent's own diagnostics as seen and recovered from, because asserting success alone would pass equally well if the outage had never reached the renewal task; an outage the controller never returns from must cancel, name the new cause, do so no sooner than five seconds into a ten-second lease where the defect cancelled at about one, and finish at or before the controller's own recorded expiry -- read from the database so it survives the controller. Gate two, which denies a renewal AT the controller, is unchanged in meaning and still passes. **Every check is mutation-proved and each turns exactly one named test red**: reverting the fix reproduces the campaign finding in gate three (`aborted`, terminal summary `lease_lost_during_execution:renewal_transport_failure`) and the wrong cause at about one second in gate four; removing the retry's lease bound makes gate four run out its own 40-second timeout; and removing the one-second margin fails the reclamation-ordering assertion by 124 ms, so that assertion is not vacuous. **Re-measured against the fix** with release binaries on the same campaign harness that found it: the controller-killed scenario goes `aborted` 2/2 to `succeeded` 2/2 with `step_starts` and `step_completions` both 1 -- the step ran exactly once -- while the both-killed and cancellation scenarios are unchanged. **One harness defect found and fixed here**: the first draft waited on `lease_owner`, which is set at the OFFER, so the kill landed in the accept path and gate three failed on an `attempt.lease_expired` that had nothing to do with the property under test; both gates now wait for `attempt.running`. **Corrections to the record**: `renewal_transport_failure` is no longer reachable as a lease-loss cause and was inaccurate for the answered refusals it also covered, which are now `renewal_refused`; `renewal_timeout` is retired into the new cause because an operator cannot act differently on a hang and on repeated unreachability. **Deliberate non-changes**: no fencing rule, no controller code, no schema, no protocol, no configuration, and no authority. Resume after AGENT death stays out of scope. **Residual, stated to be attacked**: the window in which the agent executes without a fresh answer grows from one renewal interval to at most one lease term; a controller answering `Internal` from a failing store is now retried rather than treated as lost authority; and a cancellation issued during an outage lands up to one lease later, which the store's `cancellation_lease_expired` path already handled. **Review found a second defect in the anchor the whole bound rests on**: the controller stamps `lease_expires_at` while a request is in flight and never on accept, yet each term was re-anchored at the instant its ANSWER arrived -- so a round trip longer than the one-second margin put the deadline past the reclaimable instant, which is exactly what the retry turns into running time. Each renewed term is now measured from the instant its own request left, and the accept path anchors on the branch that actually re-stamped the lease. That one is NOT mutation-proved and says so: the anchor is a call-site choice and the ordering assertion only catches it above a one-second round trip, so what is committed is the arithmetic as an executable test plus the reasoning at both call sites. **Gate five, also from review**, covers the far side of the deadline: the controller returns only after the lapse, a second enrolled runtime claims the requeued attempt at a higher fence, and the returning original agent moves nothing and is told `fenced authority is disowned` through the `EXEC-003` discharge disposition. `JCOMP-003` gains the dependency edge this ticket's runtime change requires. **One further residual named**: the first term is granted for the controller's lease and measured against the agent's, so a longer agent lease overshoots by the difference on that term only; both default to 30 s and closing it needs a protocol field or a configuration invariant. **A third review finding, and the sharpest**: cancelling an execution is not stopping it. `terminate_and_prove_group_empty` signals the group, sleeps the configured termination grace, and only then sends `SIGKILL`, so a workload that ignores `SIGTERM` kept executing for that whole grace after the agent decided to cancel -- at the shipped default, a full second past the instant the attempt becomes reclaimable, and the setting has no upper bound. Reachable before only by accident, when a refused renewal landed near expiry; the retry is what puts the cancellation at the end of the lease every time. The term now reserves the grace as well as the RPC margin, so a workload signalled at the deadline is dead before the lease expires, and the agent's configuration refuses one where the renewal cadence plus the grace does not fit inside the lease -- every configuration in this repository was checked against that rule before it landed. Both halves are mutation-proved, including the accepting case, so the rule refuses a configuration rather than a cadence. Closure receipt `docs/evidence/AGENT-007_SECURITY_REVIEW.md`. |
