...
| Runtime effect integration | `EXT-002` | ACTIVE | Exact-head Mario passes; review/CI/merge remain | Exact head `6f737080cf7546e1982fd45c2283663d941f4448` provides the typed connector-intent path, exact deployment mapping admission, shipped-format downstream digest matching, closed protected-reference validation, all-fence redacted public effect evidence, frozen-predecessor binding, fenced ambiguity handling, no-lease reconciliation receipt completion, real-PostgreSQL timeout/substitution/crash/lease/cancellation/reconciliation/zero-duplicate proofs, and a final-server PostgreSQL rehearsal gate. The bundle-backed Mario run passed all 17 tests; its owner-only result SHA-256 is `733f870961474d0be581d9aba46b244a0fc767b4c680bd4aac96c115d39163ac` and every production/canary/cutover authority flag is false. Independent exact-head review, protected CI, merge, and protected-main verification remain. |
...
PostgreSQL. The bundle-backed exact-head Mario rehearsal passed all 17 tests
with owner-only result SHA-256
