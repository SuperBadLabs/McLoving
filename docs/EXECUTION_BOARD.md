...
|---:|---|---|---|
| 1 | `EXEC-005` | PENDING | Wire sealed helpers through the product path before SECRET-002, SEC-005 and CASE-001 |
...
before the paired campaign. `JCOMP-001`, `JCOMP-002` and `JCOMP-002A` are `DONE`; `JCOMP-002B` and `AGENT-007` are `DONE`, and `JCOMP-002C` is `DONE`; `JCOMP-003` has earned its separately reviewed contained execution claim; `EXEC-005` is the next selected PENDING ticket.
