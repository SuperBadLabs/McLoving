...
| Reserved log chunk custody | `AGENT-011` | PARALLEL | `PAR-013` merged | Standalone pull request keeping a copy of every reserved live-log chunk under the agent's journal directory until its receipt, so a workload that unlinks or renames its own spool cannot strand a reserved sequence; no protocol change |
