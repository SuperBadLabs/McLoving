...
| Discovery | `DISC-001` | SERIAL | `TRIG-001`, `SCM-001`, `AUTHZ-001` are done | Active; integrates only the exact merged ingress, source, and policy contracts |
...
PR #45 is merged at protected-main commit `c9e295a5`; exact-head checks,
independent review, closure of all forty-five review threads, and post-merge
Foundation and Windows verification are complete. This board-advance change
closes `TRIG-001` and activates the multibranch and organization-folder
discovery boundary.

1. `DISC-001` — multibranch and organization-folder discovery over the exact
   merged ingress, source-acquisition, and authorization contracts.
...
persistent-Windows, DEP-001, CACHE-001, OBS-001, JOBSTATE-001, and TRIG-001
campaigns are closed. `DISC-001` is the active successor.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `DISC-001` | ACTIVE | `DIFF-003` and discovery-dependent qualification |
