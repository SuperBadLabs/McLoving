...
| MIG-007 | PENDING | MIG-006 | Generate a reviewable migration package containing canonical strict YAML, provenance, diagnostics, a mapping lock, exact source/oracle/profile/compiler digests, and a bounded bidirectional state mapping/import plan for every admitted build-number, previous-result, cross-build artifact, retained workspace, or persistent-state dependency. The package must round-trip to identical IR, contain no credential material, expose every substitution and unsupported boundary explicitly, and bind immutable source-state/export, forward and reverse transform, destination-state, and verification digests for both cutover import and rollback reconciliation. Before `DONE`, execute both directions against disposable exact-profile Jenkins and McLoving instances with seeded history: import the state, run a McLoving state-authoritative but externally effect-free build, freeze new work, reverse-reconcile its build number, result, artifacts, retained workspace/state, and audit linkage, and prove Jenkins resumes without stale lookups, duplicate mappings, missing artifacts, or duplicate effects. Every stateful job requires a successful case-specific rehearsal before `MIG-008` may grant it production effect authority. |
