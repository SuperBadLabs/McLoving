...
advanced to `CASE-001`. `DEPLOY-001` has a **bounded closure from 2026-08-27**:
...
under a controlled disposable user manager. The dispatch advances to
`EXEC-005`.
...
a particular workflow definition. Candidate workflow and verifier changes
therefore remain inside the reviewed source boundary.
...
- The six granular Foundation contexts plus the `Foundation` and `Windows`
  ...
  workflow/verifier change, review-thread resolution, exact commit, clean
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Merge authority | `CI-003` | PARALLEL | `CI-002` done | Make complete Foundation and classified Windows outcomes app-bound merge authority while retaining the six granular Foundation contexts as defense in depth; this grants repository merge gating only, no product or production authority |
...
| CI-003 | ACTIVE | CI-002 | Establish complete, app-bound merge authority. The pre-ticket branch protection required six Foundation child contexts, omitted `Backup and restore`, `Deployment lane`, and Windows, and left four contexts unbound to a reporting app. Add one always-running Foundation aggregate over every Foundation lane and one always-running Windows aggregate that accepts only classifier success plus either required-Windows success or an intentional non-impact skip. Mutation-test both aggregates, including valid `if` and `continue-on-error` fail-open controls, and lint their workflow structure with digest-verified actionlint. Retain the six granular Foundation contexts as defense in depth, bind all eight contexts to GitHub Actions under strict branch protection, and retain admin enforcement and conversation resolution. App binding authenticates the reporter rather than a candidate-controlled workflow definition, so every workflow/verifier change still requires independent exact-head review; the single-member private repository cannot enforce a second human approval without deadlocking all owner-authored changes. Closure requires both aggregates on the exact reviewed head, proof that every omitted, failed, cancelled, skipped, or invalid combination fails closed, a before/after protection readback, protected-main verification, and `docs/evidence/CI-003_SECURITY_REVIEW.md`. |
...
|---:|---|---|---|
| 1 | `EXEC-005` | PENDING | `DEPLOY-003` closed the manager-query, complete-source-union, transition-lock, PID-identity, and per-PR controlled-systemd gaps. `EXEC-005` is the earliest ready successor; it puts the already sealed helpers into the production execution spine and then unblocks `SECRET-002`, followed by `SEC-005` |
| 2 | `CI-003` | ACTIVE | Independent repository-integrity correction in PR #116; complete app-bound merge authority protects every product successor without changing their dependency order |

One pending product dispatch slot and one independent active repository-
integrity slot are selected. `EXEC-005` remains the product-critical selection;
`CI-003` is active in PR #116. `DEPLOY-003` replaced
the bounded hand-run-only evidence with the controlled per-PR manager arm and
closed the manager-query, union-validation, install-lock, PID-identity, and
measured hardening obligations recorded in its receipt. `EXEC-005` is now the
earliest ready successor. `SECRET-002` follows it, and `SEC-005` follows both,
so helper integration evidence is produced before the broker and containment
boundaries consume it.
