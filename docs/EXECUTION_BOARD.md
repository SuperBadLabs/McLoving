...
| Dependencies | `DEP-001` | SERIAL | `SCM-001` is done | Architecture-repair candidate `b6f9a0138976d3457c1a3a478c7bfee521ec7adf` replaces transient and durable dynamic directory trees with exclusive regular archives, closes the two repeated creation-to-open namespace races, and marker-scans the authenticated permanent commit before persistence and after replay loading. All 115 focused tests pass; predecessor documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passed workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, thread closure, merge, and protected-main verification remain required. The complete audit contains 125 actionable findings across 130 important seams. Mario's sealed denominator still grants zero workload dependency or repository authority, so cache, production dependency, and later authority-transfer gates remain separate. |
