...
| External clients | `CONSUMER-001` -> `ADMIN-001` | SERIAL | `AUTHZ-001` is done | Separate PRs; close read compatibility before adding the higher-authority administrative write surface |
...
1. `CONSUMER-001` — inventory and close authenticated read-side compatibility
   before the slot advances to the higher-authority `ADMIN-001` write surface.
...
core differential, identity lifecycle, authorization mapping, and
persistent-Windows work. IDP-001 and AUTHZ-001 each have independent security
and executable restore receipts, so the lane advances to external read-side
consumer migration. The persistent-Windows campaign is closed.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `CONSUMER-001` | ACTIVE | `ADMIN-001` and zero residual Jenkins reads |
