...
| Production qualification | `CASE-001` -> `CANARY-001` -> `MIG-008` | SERIAL | `CANARY-000` protected-main verification complete; `EXT-002` merged as `03a1f5d`; `SEC-005`, `EXEC-005`, and `SECRET-002` complete, because the case certifies a runtime all three change. The `CASE-001` -> `CANARY-001` transition additionally gates on `CANARY-002`, `REL-003`, and `DEPLOY-002`: the first real external effect must not be granted before the ceremony driver exists, before the release that signs it is complete, and before that release is proved installable | Close the runtime effect-integration gap, then the owner designates and fully certifies one real effectful case before the separately authorized one-action ceremony |
