...
| Artifact collection work budget | `AGENT-012` | PARALLEL | `PAR-014` merged | Standalone pull request precompiling a stage's artifact patterns once and bounding the walk's aggregate matching work, and probing a zero-length collected file for growth before its header-only stream closes; no protocol change |
