...
| 2 | `PAR-003` | ACTIVE | Human role grants: the admin tool bootstraps a project's first Owner, a project principal grants and revokes roles through the memberships route, Owner manages Owner, the last Owner stays, a revocation fences the identity's sessions; `PAR-005` stays in slot 1 until its ten matching verdicts are recorded. Successors: `PAR-002`, `PAR-015`. |
...
<!-- board-graph: allow PAR-002 ~ PAR-005 -- PAR-005's remaining work is evidence collection on the owner's host; PAR-002 (cron triggers) changes the controller's schedule worker, which the evidence deployment does not use; ordering stays with the parity lane's dispatch order -->
<!-- board-graph: allow PAR-003 ~ PAR-005 -- PAR-005 is merged and stays ACTIVE only for its ten-verdict evidence window; PAR-003 (role grants) touches identity and memberships, disjoint from the dogfood scripts, and its technical dependency is PAR-004; the parity lane's dispatch order is unchanged -->
<!-- board-graph: allow PAR-005 ~ PAR-015 -- PAR-005's remaining work is evidence collection on the owner's host; PAR-015 (build workspace affinity) changes scheduler and agent code the evidence deployment does not depend on; ordering stays with the parity lane's dispatch order -->
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity evidence | `PAR-005` | PARALLEL | merged as `1b0da107` on 2026-09-12 | The deployment on the owner's host polls `main` and builds every push; `scripts/dogfood/verdicts.sh` appends one row per push to the evidence table and the ticket closes on ten consecutive matches, with a mismatch resetting the count; no further code is planned on the ticket, review findings past the cap live in `DOGFOOD-001` |
| Product parity | `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-005` merged as `1b0da107` and `ACTIVE` for its ten-verdict evidence window; `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba`, `PAR-012` as `e22a94ed`, `PAR-001` as `327a032a`, `PAR-013` as `0e2cf213`, `PAR-014` as `9208e4e2` and `PAR-004` as `bf47e751`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
