...
|---:|---|---|---|
| 1 | `PAR-011` | ACTIVE | Container stages under rootless podman; `PAR-012` checkout follows |
...
| Container store identity | `AGENT-009` | PARALLEL | `PAR-011` merged | Standalone pull request binding the effective podman storage identity (graph root, run root, driver) into every container invocation and the journaled container context, so a storage configuration edit between launch and reap cannot make a fresh store's empty answer pass as proof; no protocol change |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-011` -> `PAR-012` -> `PAR-001` -> `PAR-013` -> `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85` and `PAR-010` as `cca42de5`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
