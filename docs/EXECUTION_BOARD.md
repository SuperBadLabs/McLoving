...
| Dependencies | `DEP-001` | SERIAL | `SCM-001` is done | Architecture-repair candidate `456cf29a5092270785c3d17b830edd6c39052117` replaces transient and durable dynamic directory trees with exclusive regular archives, closes the two repeated creation-to-open namespace races, marker-scans the authenticated permanent commit before persistence and after replay loading, carries one marker scanner across the complete transport plan, carries one stateful guard across the exact generated archive serialization, routes final archive-sync failure through exact cleanup or poisoned ambiguity, and serializes complete fetches so no overlap can succeed after a post-create poison transition. All 119 focused tests pass, including production-wired cross-artifact, header-to-payload, sync-failure, every post-create inspection/identity-failure, second-fetch denial, and overlapping-fetch proofs; predecessor documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passed workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, thread closure, merge, and protected-main verification remain required. The complete audit contains 134 actionable findings across 139 important seams. Mario's sealed denominator still grants zero workload dependency or repository authority, so cache, production dependency, and later authority-transfer gates remain separate. |
...
| DEP-001 | ACTIVE | SCM-001, SEC-003 | Implement policy-bound workload dependency resolution for Maven/npm/PyPI and other admitted ecosystems. Bind repository identity and trust policy, package coordinate, exact version, lockfile, transitive graph, content and signature/attestation digests, resolver/toolchain, credential grant, and audit provenance; mutable or unresolved coordinates are ineligible. Prove missing, repository/package/graph substitution, compromised mirror, untrusted-source, credential leak, offline/replay, and later-resolution denial before any dependency-resolving canary or cutover. Architecture-repair candidate `456cf29a5092270785c3d17b830edd6c39052117` replaces both dynamic directory hierarchies with one exclusive contiguous transport archive and one sealed atomic publication archive, marker-scans the authenticated permanent commit before persistence and after replay loading, carries one marker scanner across the complete transport plan, carries one stateful guard across the exact generated archive serialization, routes final archive-sync failure through exact cleanup or poisoned ambiguity, and serializes complete fetches so no overlap can succeed after a post-create poison transition. It passes 119 focused tests, including production-wired cross-artifact, header-to-payload, sync-failure, every post-create inspection/identity-failure, second-fetch denial, and overlapping-fetch proofs; predecessor documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passed workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, resolution of fixed threads, merge, and protected-main verification remain required before `DONE`; the current audit is 134 actionable findings across 139 important seams. Closure evidence: `docs/evidence/DEP-001_SECURITY_REVIEW.md`. Mario's sealed denominator contains zero admitted workload dependencies, so production dependency resolution, canary, cutover, rollback, and decommission authority remain separately gated. |
