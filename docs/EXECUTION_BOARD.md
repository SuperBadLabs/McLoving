...
| Source acquirer throughput and retained state | `AGENT-013` | PARALLEL | `PAR-005` merged | Standalone pull request reading blobs in one batched git process instead of one process per file and making a killed acquisition's retained state recoverable by the next; no protocol change |
