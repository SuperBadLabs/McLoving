...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Production qualification | `CANARY-001` -> `MIG-008` | SERIAL | completed `SHADOW-001`, `MIG-007`, and named production substrates | Narrowly granted canary, then receipt-only closure; never overlap authority states |
...
complete for that exact disabled case; CANARY-001 is the active serial ticket,
and no production effect authority has yet been granted.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `CANARY-001` | ACTIVE | `MIG-008`, then `CUTOVER-001` |
