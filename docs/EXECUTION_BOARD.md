...
`GROOVY-001`, `HYG-003` and `AGENT-007` remain pending parallel lanes outside
...
| Lease survival | `AGENT-007` | PARALLEL | `EXEC-004` is done | Standalone agent pull request separating a renewal the controller REFUSED from one it never answered, and retrying only the second until the held lease lapses. Nothing is proved yet: the ticket must deny the network rather than the renewal, show a short outage finishing the step and a long one cancelling under a new named cause, show the post-lapse return refused by a check it names, and leave gate two of the `EXEC-004` lease suite unchanged in meaning |
