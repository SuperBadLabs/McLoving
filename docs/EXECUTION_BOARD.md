...
Jenkins authority. `CANARY-000` resumes as the effect-free qualification
foundation in PR #70; `CANARY-001` remains the production ceremony and any
production effect still requires a separate fresh
...
| Runtime effect integration | `EXT-002` | PENDING | `CANARY-000` must merge and verify first | Add a typed connector-intent step and controller-owned, fenced dispatch/reconciliation path. The runtime must durably prepare the exact intent before dispatch, call only an independently deployed certified `EXT-001` service, checkpoint the signed outcome, replay it to the effect-free shadow, and withhold downstream execution until the observer/replay join is durable. Native process steps remain unable to reach production endpoints or credentials. |
...
| Production qualification | `CANARY-000` -> `EXT-002` -> `CANARY-001` -> `MIG-008` | SERIAL | `ALPHA-001` accepted; all named contained CANARY substrate dependencies are done | Merge PR #70's effect-free verifier foundation, then close the runtime effect-integration gap before selecting and certifying one real effect. The live action remains separately gated and owner-authorized. |
...
| CANARY-000 | ACTIVE | SHADOW-001, REL-001, EXT-001, OBS-001, DISC-001, DEP-001, CACHE-001, PROV-001 | Deliver PR #70's effect-free `mcloving-canary-qualification` foundation. It verifies seven independently signed pre-action gates, a connector-signed single dispatch time after grant issuance, a shadow-replayer-sampled signed replay-completion time taken after its durable replay claim, exact EXT-001/shadow/OBS-001 receipt joins, one-action quotas, optional Windows interruption evidence, and the independently signed final authority ledger without granting authority. Its sealed-inventory test must continue to prove that the current Mario population has zero eligible production canaries. Closure requires exact-head review, protected checks, protected-main merge, and post-merge verification; it cannot perform or authorize a production action. |
...
| EXT-002 | PENDING | CANARY-000, EXT-001, OBS-001, SHADOW-001 | Integrate the certified effect boundary into the shipped product path described by `docs/architecture/RUNTIME_EFFECT_INTEGRATION_V1.md`. Extend Pipeline IR and the execution protocol with a typed connector-intent step whose canonical request contains no raw credential, endpoint override, or arbitrary command. The controller—not an agent process—must durably prepare the exact fenced intent, obtain a fresh one-action grant, invoke only an independently deployed and runtime-attested `EXT-001` service, persist the signed outcome or ambiguity, join the independent `OBS-001` observation, feed the exact outcome to the deny-authority shadow, and release downstream nodes only after the complete durable join. Prove lease loss, controller crash, dispatch timeout, duplicate delivery, stale grant, configuration drift, response substitution, reconciliation, and restart behavior against real PostgreSQL with zero duplicate effects. Native process execution must remain network/credential/effect denied. Before `DONE`, run one effect-free exact-head Mario rehearsal against a non-production fixture destination; the first production action remains owned exclusively by `CANARY-001` and requires fresh explicit owner authorization. |
...
board on 2026-08-17. PR #70 is again the active effect-free `CANARY-000`
qualification foundation. No production effect authority has been granted;
a 2026-08-17 static call-path audit also found that Pipeline IR emits only
native process steps and that the durable effect-checkpoint API has no shipped
controller or agent caller. The standalone `EXT-001` service and `CANARY-001`
verifier therefore cannot yet form an end-to-end product effect path. `EXT-002`
records that missing integration; hand-assembled ceremony receipts cannot
substitute for it. The current Mario inventory still has zero eligible
production canaries, and any real one-action effect requires separate fresh
...
|---:|---|---|---|
| 1 | `CANARY-000` | ACTIVE | Merge effect-free PR #70, execute `EXT-002`, then certify and separately authorize one `CANARY-001` action before `MIG-008` |
