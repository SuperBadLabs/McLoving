...
## One-day product priority override

For the working day beginning 2026-08-16, `ALPHA-001` is the sole definition
of progress. The broad migration/authority sequence is paused, CANARY-001 is
deferred, and its open foundation pull request must not merge during this
override. No additional migration ticket, contained proof, review iteration,
or documentation-only closure counts as forward movement unless it directly
enables the one-command end-to-end alpha on Mario. The override ends only when
the owner accepts a retained passing demo or explicitly changes the priority.
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product alpha | `ALPHA-001` | SERIAL | `E2E-001`, `UX-002`, `UI-001`, and `AUDIT-001` | Sole one-day priority: one clean-checkout command must run and retain the accepted native product journey on Mario |
| Production qualification | `MIG-008` | SERIAL | deferred `CANARY-001` must first resume and complete | Migration and production-authority work is paused during the ALPHA-001 override |
...
| ALPHA-001 | ACTIVE | E2E-001, UX-002, UI-001, AUDIT-001 | Deliver the operator-facing native alpha defined in `docs/ALPHA_DEMO.md`: one clean-checkout command on Mario builds the shipped controller/CLI, starts isolated PostgreSQL, applies and executes the three-stage public-API pipeline, exposes UI/OpenAPI/status/graph/log/audit surfaces, restarts without losing the completed build, proves idempotent resubmission without duplicate execution, and retains an owner-only self-manifested evidence package with zero production or Jenkins authority. This is the sole progress metric during the one-day priority override. |
...
| CANARY-001 | DEFERRED | SHADOW-001, REL-001, EXT-001, OBS-001, DISC-001, DEP-001, CACHE-001, PROV-001 | Owner-paused for the ALPHA-001 one-day product override. The existing effect-free qualification foundation and PR #70 remain available for later resumption, but must not merge or execute a production effect during this override. When resumed, prove graduated per-job effect authority one action at a time under bounded quotas, retention, audit, failure thresholds, and abort rules. Before each grant, satisfy and bind the current pre-action threat-model receipt, live inventory reconciliation, quiescence proof, complete runtime/input/authority freeze, exact intent match, deny-authority shadow replay, independently observed destination state, ambiguity freeze, and any required persistent-Windows proof. |
...
complete for that exact disabled case. The owner has paused the broad migration
sequence and deferred CANARY-001; its open PR #70 must not merge during the
one-day override. ALPHA-001 is the only active ticket and a retained passing
native product demo on Mario is the only accepted progress. No production
effect authority has been granted.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `ALPHA-001` | ACTIVE | Owner acceptance, then explicit reprioritization |
