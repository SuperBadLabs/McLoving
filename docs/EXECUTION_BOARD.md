...
| Deployment validation surface | `DEPLOY-003` | SERIAL | `DEPLOY-001` | Bound the lane's validation surface to the decided trust boundary: ask the manager instead of modelling it, validate the load-path union instead of resolving the selection, and teach the suite to exercise the query path it currently skips |
