...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Identity | `AUTHZ-001` | SERIAL | `IDP-001` is done | Implementation and recovery pass; independent exact-head review remains before DONE |
...
1. `AUTHZ-001` — implementation and recovery are complete; independent
   exact-head security review remains before the slot advances to `CONSUMER-001`.
...
core differential, identity lifecycle, and persistent-Windows work. IDP-001's
independent security and executable restore receipt is attached, so its lane
advances to authorization mapping. The persistent-Windows campaign is closed.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `AUTHZ-001` | ACTIVE | Identity-dependent discovery and client migration |
