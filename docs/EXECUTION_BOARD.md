...
is now the active serial ticket. `CANARY-001` remains the production ceremony,
and any production effect still requires a separate fresh one-action owner
authorization and every pre-action gate.
...
| Runtime effect integration | `EXT-002` | ACTIVE | Exact-head Mario passes; review/CI/merge remain | Rehearsal head `6f737080cf7546e1982fd45c2283663d941f4448` provides the typed connector-intent path, exact deployment mapping admission, shipped-format downstream digest matching, closed protected-reference validation, all-fence redacted public effect evidence, frozen-predecessor binding, fenced ambiguity handling, no-lease reconciliation receipt completion, real-PostgreSQL timeout/substitution/crash/lease/cancellation/reconciliation/zero-duplicate proofs, and a final-server PostgreSQL rehearsal gate. The bundle-backed Mario run at `6f73708` passed the 17 real-spine tests that existed at that head; its owner-only result SHA-256 is `733f870961474d0be581d9aba46b244a0fc767b4c680bd4aac96c115d39163ac` and every production/canary/cutover authority flag is false. The branch has since advanced past `6f73708` with the independent-review fixes; the rehearsal was not re-run at the final head, which instead passed the complete local pinned PostgreSQL gate with the grown 24-test real-spine suite. Independent exact-head review, protected CI, merge, and protected-main verification remain. |
...
| Production qualification | `EXT-002` -> `CANARY-001` -> `MIG-008` | SERIAL | `CANARY-000` protected-main verification complete; `EXT-002` active | Close the runtime effect-integration gap before selecting and certifying one real effect. The live action remains separately gated and owner-authorized. |
...
| EXT-002 | ACTIVE | CANARY-000, EXT-001, OBS-001, SHADOW-001 | Integrate the certified effect boundary into the shipped product path described by `docs/architecture/RUNTIME_EFFECT_INTEGRATION_V1.md`. Rehearsal head `6f737080cf7546e1982fd45c2283663d941f4448` includes the typed connector-intent and execution-spec path, strict deployment-owned mapping ID/digest resolution, immutable prepared/outcome/observation/reconciliation/shadow truth, shipped-format downstream digest comparison, closed protected-reference validation, all-fence redacted API evidence, digest-pinned services, frozen pre-action predecessor binding, signed response-substitution denial, no-lease receipt completion under explicit reconciliation, lease/crash/timeout/cancellation/retry/ambiguity handling with exactly one fixture dispatch, and a PostgreSQL final-server readiness gate. The bundle-backed Mario rehearsal at `6f73708` passed the 17 real-spine tests that existed at that head (the branch has since advanced with the independent-review fixes, un-rehearsed owner-only but green on the full local pinned gate with the grown 24-test real-spine suite); owner-only result SHA-256 `733f870961474d0be581d9aba46b244a0fc767b4c680bd4aac96c115d39163ac` and checksum-file SHA-256 `c170cdbaae81aea781f6c46721e64c30ef2ee6e966c6f5f7293bb72b829470d7` bind exact-head, clean-teardown, and all-false production/canary/cutover authority evidence. Closure now requires independent exact-head review, protected CI, thread resolution, squash merge, and post-merge protected-main verification. Native process execution remains network/credential/effect denied, and the first production action remains exclusively a separately authorized `CANARY-001` ceremony. |
...
effect authority has been granted. The 2026-08-17 static call-path gap is
implemented and rehearsed at head `6f737080cf7546e1982fd45c2283663d941f4448`:
typed connector intent, deployment mapping resolution, frozen-predecessor
binding, shipped-format digest comparison, closed protected-reference
validation, fenced durable receipt joins, no-lease reconciliation completion,
all-fence redacted API evidence, and the adverse failure matrix pass against real
PostgreSQL. The bundle-backed Mario rehearsal at `6f73708` passed the 17
tests that existed at that head — the branch has since advanced with the
independent-review fixes, covered by the full local pinned gate (24 real-spine
tests) rather than a re-run owner-only rehearsal — with owner-only result
SHA-256
`733f870961474d0be581d9aba46b244a0fc767b4c680bd4aac96c115d39163ac`,
complete teardown, and every authority flag false. `EXT-002` remains active for
exact-head review, protected CI, merge, and protected-main verification. The current Mario inventory still has
zero eligible production canaries, and any real one-action effect requires separate fresh
owner authorization after `EXT-002` and the case-specific migration/shadow
prerequisites are complete.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `EXT-002` | ACTIVE | Integrate and verify the effect runtime, then certify and separately authorize one `CANARY-001` action before `MIG-008` |
