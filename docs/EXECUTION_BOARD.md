...
|---:|---|---|---|
| 1 | `PAR-004` | ACTIVE | Notifications: one terminal outbox event per build, a delivery ledger with backoff, GitHub commit status and signed HTTPS targets bound by deployment mappings; `PAR-005` dogfood follows |
...
| Commit-status reconciliation and NAT64 prefixes | `CTRL-007` | PARALLEL | `PAR-004` merged | Standalone pull request reconciling a commit status against the target after the quiet interval, so a write the target applied after the local deadline is corrected, and letting the operator name the deployment's network-specific NAT64 prefixes so their embedded IPv4 addresses are decided by the address policy; no protocol change |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba`, `PAR-012` as `e22a94ed`, `PAR-001` as `327a032a`, `PAR-013` as `0e2cf213` and `PAR-014` as `9208e4e2`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
