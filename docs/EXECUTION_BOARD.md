...
| Dependencies | `DEP-001` | SERIAL | `SCM-001` is done | Architecture-repair candidate `f602618eae3f299c2affeee6f2b8185108c784f4e` replaces transient and durable dynamic directory trees with exclusive regular archives, closing the two repeated creation-to-open namespace races. All 114 focused tests pass; documentation head `8d15ca537db6ccaea91fe041514f4b6de76bdbf7` also passes workspace-wide strict Clippy, the complete non-source workspace, exact-capacity resolver journey, board verification, and serialized AppArmor source suite on HeMan. Protected workflows, fresh independent exact-head review, thread closure, merge, and protected-main verification remain required. The complete audit contains 124 actionable findings across 129 important seams. Mario's sealed denominator still grants zero workload dependency or repository authority, so cache, production dependency, and later authority-transfer gates remain separate. |
