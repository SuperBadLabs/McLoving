...
1. `MIG-005A` corrective closure — the earlier synthetic fixture did not close
   the exact admitted job's retained `build-history` dependency. Recover the
   source object, implement and verify both directions, and complete the
   case-specific rehearsal.
