...
|---:|---|---|---|
| 1 | `PAR-014` | ACTIVE | Artifacts streamed from the agent over the existing mTLS channel and committed through the store's upload path; `PAR-004` notifications and `PAR-005` dogfood follow |
...
| Sequential planner artifact refusal | `CTRL-006` | PARALLEL | `PAR-014` merged | Standalone pull request making the sequential planner refuse a stage that declares artifacts, as it refuses a container stage, instead of planning nodes with the declarations dropped; no protocol change |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba`, `PAR-012` as `e22a94ed`, `PAR-001` as `327a032a` and `PAR-013` as `0e2cf213`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
