...
| External clients | `ADMIN-001` | SERIAL | `CONSUMER-001` is done | Migrate the sealed client's higher-authority administrative write surface in its own PR |
...
1. `ADMIN-001` — migrate the sealed `owner-operator` client's higher-authority
   administrative/write slice after the completed CONSUMER-001 read contract.
...
|---:|---|---|---|
| 1 | `ADMIN-001` | ACTIVE | zero residual Jenkins writes and `DIFF-003` |
