...
`GROOVY-001` and `HYG-003` retain independent parallel lanes outside
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Claim staleness | `HYG-003` | PARALLEL | `HYG-002` is done | Standalone hygiene pull request generalising the existing protected-main pin rule to the class review measured. Each check must be red against real pre-fix text taken from history and green against receipts that record observations correctly; cases left to written convention must be named rather than represented as gated |
