...
|---:|---|---|---|
| 1 | `JCOMP-001` | PENDING | Lock the syntax/profile/fixture and comparison contract, then `JCOMP-002` generalizes compilation and admission, then `JCOMP-002A` delivers sequential step execution, `JCOMP-002B` delivers contained workspace continuity, and `JCOMP-003` earns the paired execution claim |
...
before the paired campaign. All five milestone tickets remain `PENDING`.
...
cannot establish runnable support. CI-004 is now closed on verified PR #125
...
|---|---|---|---|---|
| Sequential Declarative support | `JCOMP-001` -> `JCOMP-002` -> `JCOMP-002A` -> `JCOMP-002B` -> `JCOMP-003` | SERIAL | `MIG-003` and `DIFF-001` done; fresh custody and Foundation checks before implementation | One contract, then compiler/admission, sequential step execution, contained workspace continuity, and paired product execution; one PR per ticket and no production authority |
...
| JCOMP-002A | PENDING | JCOMP-002 | Deliver actual sequential execution of every literal shell step under the JCOMP-001 contract through the controller and agent, preserving stage/step identities and durable terminal results. Each step uses a fresh shell; environment changes and working-directory changes do not leak to later shells. Prove first/middle/last-step failure stops later steps and stages, with explicit skipped results and correct build aggregation. Preserve lease/fence, cancellation, retry and recovery semantics with focused tests and independent review. Removing the one-step admission guard or concatenating commands into one opaque shell is insufficient. Keep unsupported workspace-dependent cases execution-ineligible until JCOMP-002B; compile-only acceptance grants no execution or production authority. |
| JCOMP-002B | PENDING | JCOMP-002A | Deliver contained build-workspace continuity across steps and stages for the JCOMP-001 contract, using an explicit build lifecycle or verified state transfer. Define agent placement/transfer and ownership, isolation between builds, bounded storage, fence/lease-loss handling, cancellation/recovery, and whole-build cleanup. Prove earlier workspace files remain available to later fresh shells, sibling builds cannot read or modify them, stale attempts cannot mutate successor state, and final outputs are observed before cleanup. Merely reusing an attempt path or disabling cleanup is insufficient. Exercise the actual controller/agent with focused lifecycle tests and independent review before broadening runnable admission. This is contained M1 support, not production containment closure or operational authority. |
