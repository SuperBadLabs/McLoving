...
| Deployment smoke cost | `CI-004` | PARALLEL | `CI-003`, `DEPLOY-003` are done | Reduce disposable smoke-fixture debug payload while retaining every assertion, checksum check, required context, and exact systemd runtime identity; PR #125 is active pending corrected-head hosted verification and independent review |
...
| CI-004 | ACTIVE | CI-003, DEPLOY-003 | Reduce the measured deployment smoke critical path in `deploy/test-deployment.sh` by removing only debug sections from disposable fixture copies before checksums are sealed. Preserve original build outputs, compiled code and assertions, symbol names and unwind data, all smoke cases, and the unchanged systemd gate's exact installed/controller byte identity. Require before/after evidence, full corrected-head Foundation and Windows checks, and independent review recorded in `docs/evidence/CI-004_SECURITY_REVIEW.md`. Precursor PR #125 head `4e93d5d` passed the full smoke matrix but failed the final systemd identity check; its systemd stripping is removed, and closure remains pending corrected-head evidence. |
