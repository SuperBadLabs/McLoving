...
| EXT-002 | ACTIVE | CANARY-000, EXT-001, OBS-001, SHADOW-001 | Integrate the certified effect boundary into the shipped product path described by `docs/architecture/RUNTIME_EFFECT_INTEGRATION_V1.md`. Extend Pipeline IR and the execution protocol with a typed connector-intent step whose canonical request contains no raw credential, endpoint override, or arbitrary command. The controller—not an agent process—must durably prepare the exact fenced intent, obtain a fresh one-action grant, invoke only an independently deployed and runtime-attested `EXT-001` service, persist the signed outcome or ambiguity, join the independent `OBS-001` observation, feed the exact outcome to the deny-authority shadow, and release downstream nodes only after the complete durable join. The active implementation has proved the positive three-role signed join and a pre-dispatch substituted-service denial against real PostgreSQL, while keeping terminal publication closed until the shadow receipt is durable. Ticket closure still requires deployment-backed unknown/floating mapping denial, redacted API evidence reads, the remaining lease/crash/timeout/duplicate/reconciliation matrix, exact-head review and protected CI, and the effect-free Mario rehearsal. Native process execution must remain network/credential/effect denied. The first production action remains owned exclusively by `CANARY-001` and requires fresh explicit owner authorization. |
...
effect authority has been granted. `EXT-002` is now active because a 2026-08-17
static call-path audit found that Pipeline IR emits only
