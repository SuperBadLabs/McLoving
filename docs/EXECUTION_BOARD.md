...
| Workload containment | `SEC-005` | PARALLEL | `DEPLOY-001` | Contain the spawned step so submitting a pipeline is not equivalent to reading the host's deployment credentials; independent of the serial authority chain |
...
|---:|---|---|---|
| 1 | `EXEC-001` | PENDING | Terminal fail-closed refusal of unsupported specs; validate-accepted must imply runnable |
| 2 | `EXEC-003` | PENDING | Name the shared-identity collision on both sides and give the operator a documented discharge verb |
| 3 | `EXEC-002` | PENDING | One capability vocabulary; embedded-worker startup fails closed when its declared capabilities cannot match |
