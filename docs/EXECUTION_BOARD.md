...
|---:|---|---|---|
| 1 | `PAR-013` | ACTIVE | Live log streaming with journaled sequence reservations and a follow cursor; `PAR-014` artifacts and `PAR-004` notifications follow |
...
| Reserved log chunk custody | `AGENT-011` | PARALLEL | `PAR-013` merged | Standalone pull request keeping a copy of every reserved live-log chunk under the agent's journal directory until its receipt, so a workload that unlinks, renames, truncates or overwrites its own spool can neither strand a reserved sequence nor pin the attempt in recovery; no protocol change |
| Log quota accounting | `CTRL-005` | PARALLEL | `PAR-013` merged | Standalone pull request keeping each attempt's committed-log byte count as one row updated under the per-attempt log lock the append already holds, so the byte quota check is a read rather than a sum over every prior chunk; no protocol change |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-013` -> `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`, `PAR-010` as `cca42de5`, `PAR-011` as `0eb949ba`, `PAR-012` as `e22a94ed` and `PAR-001` as `327a032a`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
