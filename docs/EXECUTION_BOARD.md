...
|---:|---|---|---|
| 1 | `PAR-010` | ACTIVE | Multi-step stages under the version-5 envelope; `PAR-011` containers follows |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Product parity | `PAR-010` -> `PAR-011` -> `PAR-012` -> `PAR-001` -> `PAR-013` -> `PAR-014` -> `PAR-004` -> `PAR-005` -> `PAR-003` -> `PAR-002` -> `PAR-015` | SERIAL | `PAR-000` merged as `aeb6dd85`; each ticket starts after its predecessor is merged and verified | One pull request per ticket in this order; the chain is the dispatch queue and the distance metric; proof commands recorded in each pull request |
