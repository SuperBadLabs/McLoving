...
| Runtime effect integration | `EXT-002` | ACTIVE | Exact-head Mario passes; review/CI/merge remain | PR #72 provides the typed connector-intent path, exact deployment mapping admission, controller-owned prepared/outcome/observation/reconciliation/shadow truth, and the real-PostgreSQL adverse matrix with every authority flag false; independent exact-head review, protected CI, squash merge, and protected-main verification remain |
...
| Production qualification | `EXT-002` -> `CASE-001` -> `CANARY-001` -> `MIG-008` | SERIAL | `CANARY-000` protected-main verification complete; `EXT-002` in review as PR #72 | Close the runtime effect-integration gap, then the owner designates and fully certifies one real effectful case before the separately authorized one-action ceremony |
...
| Performance | `PERF-001` | PARALLEL | `MIG-006`, `REL-001` done; `EXEC-004` closed first so long-step envelopes measure work, not the lease defect | Isolated reproducible capacity lane; may run while later migration gates advance |
...
the three-mutable-PR cap. `EXEC-004` outranks every other hardening ticket:
until the lease renews during a step, any real workload longer than one lease
term wedges the production remote lane.

| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Lease-during-step truth | `EXEC-004` | PARALLEL | none | Renew the lease concurrently with step execution; gate one step spanning at least three lease terms to terminal success; re-prove lease-loss cancellation by blocking renewal deliberately |
| Unsupported-spec admission | `EXEC-001` | PARALLEL | none | Validate-accepted must imply runnable; a claim-time refusal is terminal and named, never an infinite retry presenting as progress |
| Capability vocabulary | `EXEC-002` | PARALLEL | none | One documented capability vocabulary; embedded-worker startup fails closed when its declared capabilities cannot satisfy the default submission |
| Identity-collision verb | `EXEC-003` | PARALLEL | none | Name the shared-identity collision on both sides and give the operator a documented resolution for an orphaned recovered attempt |
| Outbox truth | `OUTBOX-001` | PARALLEL | none | The shipped controller must drain what it writes, or the outbox must be explicitly retired; unbounded unpublished rows are not a neutral state |
| Workspace hygiene | `HYG-001` | PARALLEL | none | Retire placeholder crates, give the object store an integration gate, and prove the OIDC insecure-loopback escape hatch is unreachable in the shipped binary |
...
| EXEC-004 | PENDING | — | **The lease is renewed between steps but never during one.** Measured (build-Jenkins duel, 2026-08-10, main `0ca0142`, luigi, production remote-agent lane): with `MCLOVING_LEASE_SECONDS=30` the agent wedged into `reconciliation_required` roughly 27 seconds into a ~200-second Maven step — deterministically, on every attempt; with the lease raised to 600 the IDENTICAL pipeline SUCCEEDED in 341 s and produced a verified `jenkins.war`. The conclusion is uncomfortable and earned: the most exhaustively engineered machinery in this repository — fenced leases, session epochs, journal-before-ack, sixteen-scheduler claim races — has never once performed the single behavior leases exist for, holding authority WHILE work happens, because no gate in the repository runs a step longer than a few seconds. Every real-world build step on earth exceeds one lease term. Acceptance: the agent renews its lease concurrently with step execution; a gate runs ONE step lasting at least three lease terms to terminal success on the remote lane; lease-loss cancellation (AGENT-004) is re-proven by a gate that BLOCKS renewal deliberately, never by a step merely outrunning it; and a long step under a short lease produces a NAMED diagnostic on agent and controller, not a silent `reconciliation_required`. Evidence: luigi `~/duel-mcloving-receipts.tgz` — env, logs, and four wrecked journals across five rounds, each labeled with what wrecked it |
...
| Ticket | Status | Depends on | Objective and acceptance |
|---|---|---|---|
| OUTBOX-001 | PENDING | CTRL-001, CTRL-003 | The controller writes outbox rows in the same transaction as every durable state change, but no shipped binary drains them: `publish_outbox` is invoked only from `execution-spine` tests, so a deployed controller accumulates `published_at IS NULL` rows without bound. Decide and implement one of exactly two outcomes: a durable at-least-once outbox publisher in `bins/controller` with a documented consumer contract, idempotent delivery, crash/restart proof in the real-PostgreSQL gate, and backlog observability; or explicit retirement of the outbox table through a reviewed migration with an updated ADR 0005 rationale proving no reader exists. Silent accumulation is not a neutral state. |
| HYG-001 | PENDING | — | Workspace hygiene with three bounded items. First, retire or absorb the placeholder crates `crates/domain` (14 lines) and `crates/state-machine` (18 lines, a stale second `AttemptPhase` definition diverging from the real phase truth in `controller-store` and `agent-runtime`); `bins/agent` must not depend on a placeholder. Second, give `crates/object-store` — the only core-plane crate with no integration test suite despite owning artifact immutability, quotas, redaction, and gap typing — a real `tests/` gate covering staged publication, quota enforcement, no-overwrite, and gap classification. Third, prove `OidcClientConfig::allow_insecure_loopback_for_tests` cannot be enabled from environment or configuration in the shipped controller binary with a permission-negative test. |
| DEPLOY-001 | PENDING | REL-001 | `CUTOVER-001` must re-read every deployed implementation digest, yet `deploy/kubernetes`, `deploy/podman`, and `deploy/systemd` are README stubs and only the AppArmor profiles are real. Implement one supported production deployment lane — systemd units or podman quadlets for the controller, agent, connector, observer, and shadow services — with digest-pinned release artifacts, split migration/runtime database roles, explicit environment contracts, health checks, and a documented install/upgrade/rollback runbook. Acceptance: a scripted install on a clean host brings up the full spine and passes the deployable-runtime gate, and the cutover freeze can read every deployed digest from this lane without hand-assembled state. |
...
| EXT-002 | ACTIVE | CANARY-000, EXT-001, OBS-001, SHADOW-001 | Integrate the certified effect boundary into the shipped product path described by `docs/architecture/RUNTIME_EFFECT_INTEGRATION_V1.md`: the typed connector-intent and execution-spec path, strict deployment-owned mapping ID/digest resolution, immutable controller-owned prepared/outcome/observation/reconciliation/shadow truth, shipped-format downstream digest comparison, closed protected-reference validation, all-fence redacted API evidence, digest-pinned out-of-process connector/observer/shadow services, frozen pre-action predecessor binding, signed response-substitution denial, no-lease receipt completion under explicit reconciliation, and lease/crash/timeout/cancellation/retry/ambiguity handling with exactly one fixture dispatch, closed by a real-PostgreSQL readiness gate. PR #72 carries the implementation with an owner-only internal-network Mario rehearsal whose production/canary/cutover authority flags are all false. Closure requires independent exact-head review, protected CI, thread resolution, squash merge, and post-merge protected-main verification. Native process execution remains network/credential/effect denied, and the first production action remains exclusively a separately authorized `CANARY-001` ceremony. |
...
the remaining campaign: the runtime effect-integration gap is implemented and
rehearsed in PR #72 (`EXT-002`, in review); the owner-run bench defects were
adopted as `EXEC-001` through `EXEC-004`; the whole-repository review added
`OUTBOX-001`, `HYG-001`, and `DEPLOY-001`; and the path to a real ceremony was
made explicit as `CASE-001` (owner-designated effectful case with fresh
per-case certification) plus `CANARY-002` (fixture-only ceremony driver). The
...
`EXT-002`, `CASE-001`, and `CANARY-002` are complete.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `EXT-002` | ACTIVE | Integrate and verify the effect runtime, then `CASE-001` and the separately authorized `CANARY-001` ceremony before `MIG-008` |
| 2 | `EXEC-004` | PENDING | Lease renewal during a step unblocks truthful `PERF-001` envelopes and every real workload on the remote lane |
| 3 | `EXEC-001` | PENDING | Terminal fail-closed refusal of unsupported specs; then `EXEC-002` and `EXEC-003` as the lane frees |
