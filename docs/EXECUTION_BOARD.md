...
| State transforms | `MIG-005A` | SERIAL | Corrective audit of the exact admitted case | Reopen the state-transform boundary: the sealed inventory records one retained `build-history` object whose forward and rollback transforms remain unsupported. The private HeMan tree reproduces the already-published inventory digest, passed a networkless zero-finding secret scan, and is owner-read-only under an internal source-seal receipt retained only on HeMan; implement both transforms and complete the case-specific rehearsal before returning to `MIG-007` |
...
| Lane | Ticket or ordered chain | Class | Start gate | Streamlined execution rule |
|---|---|---|---|---|
| Certification join | `MIG-007` | SERIAL | `MIG-005A`, `MIG-006` | Immutable aggregate closure is complete; generate the reproducible migration package in its separate PR |
...
1. `MIG-005A` corrective closure — the earlier synthetic fixture did not close
   the exact admitted job's retained `build-history` dependency. The private
   source tree is digest-matched, zero-finding secret-scanned, and owner-read-only
   sealed; implement and verify both directions and complete the case-specific
   rehearsal.
2. `MIG-007` — regenerate the package only after that corrective closure; the
   current fail-closed package truthfully rejects all 228 cases and grants no
   shadow or authority eligibility.
...
| MIG-007 | PENDING | MIG-005A, MIG-006 | Generate a reviewable migration package containing canonical strict YAML, the exact reviewed `JOBSTATE-001` operational-state record, provenance, diagnostics, a mapping lock, exact source/oracle/profile/compiler digests, and the exact already-certified `MIG-005A` bidirectional state transforms plus `MIG-006` seeded-history differential and rehearsal receipts for every admitted state dependency. The package must round-trip to identical IR and operational state, contain no credential material, expose every substitution and unsupported boundary explicitly, bind immutable source export, forward/reverse transform, destination state, and verification digests for cutover and rollback, and reproduce the packaged receipt verification without invoking alternative transform logic. The current v1 implementation is deliberately incomplete: it authenticates the exact retained `build-history` dependency, records unsupported transforms, rejects the sole compiler-admitted case with `E_STATE_TRANSFER_EVIDENCE_UNAVAILABLE`, and cannot advance to `SHADOW-001`. |
...
The exact-case audit reopened MIG-005A as the active serial ticket; MIG-007 is
pending its corrective closure.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `MIG-005A` | ACTIVE | `MIG-007`, then `SHADOW-001` |
