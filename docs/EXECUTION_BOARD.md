...
1. `DIFF-003` — `REL-001` is complete, so every named parity-substrate
   prerequisite is frozen and the final external-boundary differential is the
   active serial ticket.
...
SECRET-001, DIFF-002, and REL-001 campaigns are closed. The signed v0.1.0
private Linux release is cryptographically verified but not deployed. DIFF-003
is now the active serial ticket.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `DIFF-003` | ACTIVE | `MIG-006`, `MIG-007`, and production qualification |
