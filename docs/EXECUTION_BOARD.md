...
| Source acquisition | `SCM-001` | PARALLEL | `SEC-003`, `AGENT-004` are done | Exact-head review reopened the isolated boundary for credential-marker completeness, verified-open executable use, retained-directory verification, and deterministic deadline proof; dependent work remains gated until the replacement head passes review, protected checks, and protected-main merge |
...
| SCM-001 | ACTIVE | SEC-003, AGENT-004 | Implement isolated live source acquisition for checkout, Git, submodule, and credentialed repository steps. Bind provider, repository identity, authenticated ref and exact revision, fork and trust policy, submodule graph, sparse/depth options, checkout implementation, scoped short-lived credential grant, and resulting content/provenance digests. Prove later-commit delivery, ref substitution and untrusted-fork denial, credential non-disclosure, replay resistance, bounded network/filesystem authority, cleanup, and differential checkout truth before any source-dependent canary or cutover. Exact-head review reopened closure with four root findings plus this board gate; all are implemented and under replacement exact-head verification, so `DEP-001`, `SECRET-001`, and `DISC-001` remain blocked until protected-main merge. |
