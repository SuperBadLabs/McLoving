...
| Web interface | `UI-002` -> `UI-003` | SERIAL | `UI-001` and `API-002` are done | Two standalone pull requests, in this order and not folded together. `UI-002` earns the browser-journey claim `UI-001` already made and cannot support, and captures the rendered baseline a replacement would otherwise be measured against nothing; it touches no served asset, route, policy or authorization. `UI-003` then decides the session and output-encoding boundary a server-rendered interface moves, and records it -- a decision pull request that ships no implementation, on the `GROOVY-001` pattern. Nothing is proved by either yet |
