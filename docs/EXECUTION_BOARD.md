...
1. `INPUT-001` — implement the isolated typed read-only adapter boundary for
   live external runtime inputs.
...
ADMIN-001 each have independent security and executable receipts; production
caller cutover remains separately gated. The persistent-Windows campaign is
closed.

| Slot | Current ticket | Status | Dependency-critical successors |
|---:|---|---|---|
| 1 | `INPUT-001` | ACTIVE | `DIFF-003` and production input-dependent qualification |
