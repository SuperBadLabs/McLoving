...
5. Start everything:
   ...
   ```sh
   systemctl --user enable --now mcloving-db-init mcloving-controller mcloving-agent
   systemctl --user start mcloving-postgres
   ```

   **`mcloving-postgres` is started, not enabled**, and this instruction said
   otherwise until a real service-managed run tried it.
   `mcloving-postgres.service` is GENERATED by podman's Quadlet generator from
   `mcloving-postgres.container`, and systemd refuses to enable a generated
   unit — *"Unit …/generator/mcloving-postgres.service is transient or
   generated."* Quadlet honours the quadlet's own `[Install] WantedBy=` itself,
   so the generated unit is pulled in by `default.target` and needs starting
   rather than enabling. `deploy/test-deployment-systemd.sh` now runs this exact
   command sequence as a gate, so the runbook and the lane cannot drift apart
   again.
