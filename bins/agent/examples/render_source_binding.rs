...
mod real {
    ...
    fn launcher(profile: Option<String>) -> Option<SourceLauncher> {
        let restricted =
            std::fs::read_to_string("/proc/sys/kernel/apparmor_restrict_unprivileged_userns")
                ...
                .unwrap_or(false);
        let profile =
            profile.or_else(|| restricted.then(|| "mcloving-source-acquirer".to_owned()))?;
    ...
    pub async fn main() {
        ...
        let intent: Intent =
            serde_json::from_slice(&std::fs::read(&intent_path).expect("read intent"))
                .expect("parse intent");
        ...
        let git_remote_https = std::fs::canonicalize(
            Path::new(&git_output(&["--exec-path"])).join("git-remote-https"),
        )
        ...
        let config = SourceConfig {
            ...
            runtime_closure_sha256: runtime_closure_digest(&runtime_closure)
...
fn main() {
    eprintln!(
        "render_source_binding renders a binding for the sealed source acquirer, which runs on unix hosts only"
    );
    std::process::exit(2);
}
